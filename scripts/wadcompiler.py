...
class Wad:
    ...
    def read(self):
        self.type = self.file.read(4).decode('latin-1')
        ...
        for i in range(count):
            ...
            name = self.file.read(8).decode('latin-1')
    ...
    def write(self):
        ...
        self.file.seek(0)
        # Type.
        self.file.write(self.type.encode('latin-1'))
        ...
        for info in info_table:
            self.file.write(struct.pack('<II', info[0], info[1]) +
                            valid_lump_name(info[2]).encode('latin-1'))
        ...
        for lump in self.lumps:
            ...
            if len(names) == 0 or nice_name.upper() in names:
                ...
                if os.path.exists(nice_name + ext):
                    ...
                    nice_name += ext
                print("extracting %s (%i bytes) as %s" % (lump.name, len(lump.data), nice_name))
                f = open(nice_name, 'wb')
                f.write(lump.data)
                f.close()
