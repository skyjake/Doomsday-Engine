...
static void error(const char *what, const char *msg)
{
...
int DS_Init(void)
{
    ...
    // Init A3D.
    if(FAILED(A3dRegister()))
        return false;
    if(FAILED(A3dInitialize()))
        return false;
...
void DS_Shutdown(void)
{
    ...
    if(a3d)
        a3d->Shutdown(); // This'll get rid of everything.
    a3d = NULL;
    a3dListener = NULL;
    a3dReverb = NULL;
    A3dUninitialize();
...
sfxbuffer_t* DS_CreateBuffer(int flags, int bits, int rate)
{
    ...
    // Create a new source.
    if(FAILED(hr =
       a3d->NewSource(play3d? A3DSOURCE_TYPEDEFAULT
                      : A3DSOURCE_INITIAL_RENDERMODE_NATIVE, &src)))
        return NULL;
...
void DS_Load(sfxbuffer_t *buf, struct sfxsample_s *sample)
{
    ...
    {
        if(verbose)
            error("DS_Load", "Failed to allocate audio data.");
    ...
    {
        if(verbose)
            error("DS_Load", "Failed to lock source.");
 ...
 */
static void setPan(IA3dSource2 *source, float pan)
{
    float       gains[2];
