...
function (deng_add_package packName)
    ...
    execute_process (COMMAND
        ${PYTHON_EXECUTABLE}
        "${DE_SOURCE_DIR}/scripts/buildpackage.py" ${fullPath} ${outDir}
        OUTPUT_VARIABLE msg
    ...
    if (NOT CMAKE_GENERATOR STREQUAL "Xcode")
        add_custom_command (OUTPUT ${outDir}/${outName}
            COMMAND ${PYTHON_EXECUTABLE} "${DE_SOURCE_DIR}/scripts/buildpackage.py"
                    ${fullPath} ${outDir}
                    DEPENDS ${packSrc}
...
function (deng_install_bundle_deps target)
    if (APPLE AND NOT IOS)
        ...
        foreach (_dep ${_deps})
            if (TARGET ${_dep})
                if (_dep MATCHES "(.*)::(.*)")
                    # Resolve ALIAS to the real imported target.
                    set (_resolved ${_dep})
                    get_property (_aliased TARGET ${_dep} PROPERTY ALIASED_TARGET)
                    if (_aliased)
                        set (_resolved ${_aliased})
                    endif ()
                    get_property (_loc TARGET ${_resolved} PROPERTY IMPORTED_LOCATION)
                    if (_loc MATCHES "\\.framework/")
                        string (REGEX MATCH "(.*/[^/]*\\.framework)" _fw_path "${_loc}")
                        install (DIRECTORY "${_fw_path}" DESTINATION ${_fwDir})
                    else ()
