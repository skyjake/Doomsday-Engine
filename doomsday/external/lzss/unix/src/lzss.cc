...
LZFILE *lzOpen(char *filename, char *mode)
{
	...
	if((f = (LZFILE*) malloc(sizeof(LZFILE))) == NULL)
	{
		errno = ENOMEM;
		return NULL;
	}
	...
	if(f->flags & LZFILE_FLAG_WRITE)
	{
		...
		{
			/* write a packed file */
			PACK_DATA *dat = (PACK_DATA*) malloc(sizeof(PACK_DATA));
	...
	{							/* must be a read */
		...
		{
			/* read a packed file */
			UNPACK_DATA *dat = (UNPACK_DATA*) malloc(sizeof(UNPACK_DATA));
...
LZFILE *lzOpenChunk(LZFILE * f, int pack)
{
	...
	if(f->flags & LZFILE_FLAG_WRITE)
	{
		/* write a sub-chunk */
		name = tmpnam(NULL);
		chunk = lzOpen(name, (char*) (pack ? F_WRITE_PACKED : F_WRITE_NOPACK));

		if(chunk)
		{
			chunk->filename = (char*) malloc(strlen(name) + 1);
	...
	else
	{
		...
		if((chunk = (LZFILE*) malloc(sizeof(LZFILE))) == NULL)
		{
			errno = ENOMEM;
			return NULL;
		}
