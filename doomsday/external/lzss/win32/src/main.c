/*
	Lempel-Ziv Compressed File I/O
	Win32 DLL
 ...
 ***************************************************
   ...
   Later still, modified to function as a stand-alone Win32 DLL by Jaakko 
   Keränen.
 ...
 */
static int pack_write(LZFILE *file, PACK_DATA *dat, int size, unsigned char *buf, int last)
{
	int i = dat->i;
	int c = dat->c;
	int len = dat->len;
 ...
 */
static int pack_read(LZFILE *file, UNPACK_DATA *dat, int s, unsigned char *buf)
{
	int i = dat->i;
	int j = dat->j;
	int k = dat->k;
...
LZFILE LZSSEXPORT *lzOpen(char *filename, char *mode)
{
	...
	for (c=0; mode[c]; c++) {
		switch (mode[c]) {
		case 'r': case 'R': f->flags &= ~LZFILE_FLAG_WRITE; break;
		case 'w': case 'W': f->flags |= LZFILE_FLAG_WRITE; break;
		case 'p': case 'P': f->flags |= LZFILE_FLAG_PACK; break;
		case '!': f->flags &= ~LZFILE_FLAG_PACK; header = TRUE; break;
		}
	}
	...
	if (f->flags & LZFILE_FLAG_WRITE) {
		...
		if (f->flags & LZFILE_FLAG_PACK) {
			...
			f->parent = NULL;
			f->pack_data = NULL;
			errno = FILE_FINDFIRST(filename, _A_RDONLY | _A_HIDDEN | _A_ARCH, &dta);
			if (errno != 0) {
				FILE_FINDCLOSE(&dta);
				free(f);
				return NULL;
...
int RefillBuffer(LZFILE *f)
{
	...
	if ((f->flags & LZFILE_FLAG_EOF) || (f->todo <= 0)) {     /* EOF */
		f->flags |= LZFILE_FLAG_EOF;
		return EOF;
	}
