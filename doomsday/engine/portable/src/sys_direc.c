...
void Dir_SetPath(directory_t* dir, const char* path)
{
    ...
    strncat(dir->path, fileName, FILENAME_T_MAXLEN - 1);
...
static void setPathFromPathDir(directory_t* dir, const char* path)
{
    ...
    strncat(dir->path, transPath, FILENAME_T_MAXLEN - 1);
#if defined(WIN32)
...
static void resolveHomeRelativeDirectives(char* path, size_t maxLen)
{
    ...
    if(path[1] == '/')
    {
        ...
        if(LAST_CHAR(buf) != '/')
            strncat(buf, "/", FILENAME_T_MAXLEN - 1);

        // Append the rest of the original path.
        strncat(buf, path + 2, FILENAME_T_MAXLEN - 1);
    }
    else
    {
        ...
        strncat(buf, path + 1, FILENAME_T_MAXLEN - 1);
    }
...
#  error Dir_mkpath has no implementation for this platform.
    ...
    do
    {
        ...
        if(!endptr)
            strncat(buf, ptr, FILENAME_T_MAXLEN - 1);
        else
        ...
        strncat(buf, DIR_SEP_STR, FILENAME_T_MAXLEN - 1);
        ptr = endptr + 1;
