...
// Subsector
    ...
    {DAM_SEG_FIRST, DAM_SUBSECTOR, DDVT_UINT, "firstseg"},
// Node
...
static void finalizeMapData(gamemap_t *map)
{
    ...
    // Look up sector number for each subsector and build subsector seg tables.
    for(i = 0, ss = map->subsectors; i < map->numsubsectors; ++i, ss++)
    {
        uint        n;
        boolean     found = false;

        ss->segs = Z_Malloc(sizeof(seg_t*) * (ss->segcount + 1), PU_LEVELSTATIC, 0);
        for(j = ss->firstWADSeg, n = 0; n < ss->segcount; ++j, ++n)
        {
            seg = &map->segs[j];
    ...
    ssecbuffer = Z_Malloc((map->numsubsectors + map->numsectors) *
                           sizeof(subsector_t*), PU_LEVELSTATIC, NULL);
    ssecbptr = ssecbuffer;
    ...
    for(i = 0, sec = map->sectors; i < map->numsectors; ++i, sec++)
    {
        if(sec->subscount > 0)
        {
            ...
            ssecbptr += sec->subscount + 1;
            sec->subsectors[sec->subscount] = NULL; // terminate
        }
    }
