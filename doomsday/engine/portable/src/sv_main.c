...
void Sv_GetInfo(serverinfo_t* info)
{
    ...
    strncpy(info->gameIdentityKey, Str_Text(Game_IdentityKey(Games_CurrentGame(App_Games()))), sizeof(info->gameIdentityKey) - 1);
    ...
    // Some WAD names.
    F_ComposeFileList(FT_WADFILE, true/*only "custom" files*/, info->pwads, sizeof info->pwads, ";");
...
void Sv_HandlePacket(void)
{
    ...
    case PCL_HELLO2:
        ...
        if(netBuffer.msg.type == PCL_HELLO2)
        {
            ...
            if(strnicmp(buf, Str_Text(Game_IdentityKey(Games_CurrentGame(App_Games()))), 16))
            {
