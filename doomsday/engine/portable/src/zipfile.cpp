...
static void ApplyPathMappings(ddstring_t* dest, const ddstring_t* src)
{
    ...
    if(Str_At(src, 0) == '@')
    {
        ...
        Str_Appendf(out, "%sauto/", Str_Text(&reinterpret_cast<de::Games*>(App_Games())->currentGame().defsPath()));
    ...
    if(Str_At(src, 0) == '#')
    {
        ...
        Str_Appendf(out, "%sauto/", Str_Text(&reinterpret_cast<de::Games*>(App_Games())->currentGame().dataPath()));
    ...
    if(strchr(Str_Text(src), '/') == NULL)
    {
        ...
        case RC_PACKAGE: // Mapped to the Data directory.
            Str_Appendf(&mapped, "%sauto/", Str_Text(&reinterpret_cast<de::Games*>(App_Games())->currentGame().dataPath()));
            break;
        case RC_DEFINITION: // Mapped to the Defs directory.
            Str_Appendf(&mapped, "%sauto/", Str_Text(&reinterpret_cast<de::Games*>(App_Games())->currentGame().defsPath()));
            break;
