 ...
 */
int Def_ReadDEDFile(const char* fn, filedirectory_pathtype_t type, void* parm)
{
    // Skip directories.
    if(type == FDT_DIRECTORY)
        return true;
...
void Def_ReadProcessDED(const char* fileName)
{
    ...
    Def_ReadDEDFile(fileName, FDT_FILE, 0);
}
 ...
 */
static int autoDefsReader(const ddstring_t* fileName, filedirectory_pathtype_t type, void* paramaters)
{
    // Ignore directories.
    if(type != FDT_DIRECTORY)
...
void Def_GenerateAutoMaterials(void)
{
    ...
    for(i = 0; i < patchCompositeCount; ++i)
    {
        ...
        mat->id = Uri_Construct2(pcomTex->name, RC_NULL);
    ...
    for(i = 0; i < flatTextureCount; ++i)
    {
        ...
        mat->id = Uri_Construct2(flatTex->name, RC_NULL);
    ...
    for(i = 0; i < spriteTextureCount; ++i)
    {
        ...
        mat->id = Uri_Construct2(sprTex->name, RC_NULL);
