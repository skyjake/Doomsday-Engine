 ...
 */
static void Huff_ResizeBuffer(huffbuffer_t *buffer, size_t neededSize)
{
    ...
    if((buffer->data = (byte*) M_Realloc(buffer->data, buffer->size)) == NULL)
...
void Huffman_Init(void)
{
    ...
    if(ArgExists("-huffcodes"))
    {
        ...
        Con_Message("Huffman codes:\n");
        for(i = 0; i < 256; ++i)
        {
            uint    k;
            realBits += freqs[i] * 8;
            huffBits += freqs[i] * huffCodes[i].length;
            Con_Message("%03i: (%07i) ", i, (int) (6e6 * freqs[i]));
...
byte *Huffman_Decode(const void *data, size_t size, size_t *decodedSize)
{
    ...
    size_t          outBytes = 0;
    const byte     *in = data;
    const byte     *lastIn = in + size - 1;
    byte            bit = 3, lastByteBits;
