...
DENG2_PIMPL(ModelDrawable)
{
    ...
    struct Mesh
    {
        ...
        QVector<BoneData> bones; // indexes by bone index
        VBuf *buffer;
         ...
         */
        void initBones(aiMesh const &mesh)
        {
            ...
            for(duint i = 0; i < mesh.mNumBones; ++i)
            {
                ...
                {
                    addVertexWeight(bone.mWeights[w].mVertexId, boneIndex, bone.mWeights[w].mWeight);
                }
            }
        }

        VBuf *makeBuffer(aiMesh const &mesh)
        {
            ...
            for(duint i = 0; i < mesh.mNumVertices; ++i)
            {
                ...
                {
                    /*qDebug() << "vtx" << i
                             << "bone:" << vertexBones[i].ids[b]
                             << "weight:" << vertexBones[i].weights[b];*/

                    v.boneIds[b]     = vertexBones[i].ids[b];
                    v.boneWeights[b] = vertexBones[i].weights[b];
                }
            ...
            qDebug() << "new GLbuf" << buffer << "name:" << mesh.mName.C_Str();
        ...
        void accumulateAnimationTransforms(ddouble time, GLUniform &boneMatrices,
                                           aiAnimation const &anim,
                                           aiNode const &rootNode,
                                           Matrix4f const &globalInverse) const
        {
            ...
            data.globalInverse = globalInverse;
            ...
            {
                boneMatrices.set(i, data.finalTransforms.at(i));
            }
        }
                                  ...
                                  Matrix4f const &parentTransform = Matrix4f()) const
        {
            ...
            if(boneIndex >= 0)
            {
                data.finalTransforms[boneIndex] =
                        data.globalInverse * globalTransform * bones.at(boneIndex).offset;
            }
    ...
    void initFromScene(aiScene const &scene)
    {
        ...
        for(duint i = 0; i < scene.mNumMeshes; ++i)
        {
            ...
            Mesh *mesh = new Mesh;
            mesh->makeBuffer(sceneMesh);
            meshes << mesh;

            // Update the total bounding box.
            for(duint i = 0; i < sceneMesh.mNumVertices; ++i)
            {
                aiVector3D const *pos = &sceneMesh.mVertices[i];
                addToBounds(Vector3f(pos->x, pos->y, pos->z));
            }
        }
    ...
    void drawNode(aiNode const &node/*, DrawState const &state*/)
    {
        ...
        {
            Mesh const &mesh = *meshes.at(node.mMeshes[i]);

            if(scene.HasAnimations())
            {
                mesh.accumulateAnimationTransforms(animTime,
                                                   uBoneMatrices,
                                                   *scene.mAnimations[0],
                                                   *scene.mRootNode,
                                                   globalInverse);
