...
void ST_doFullscreenStuff(int player)
{
    ...
    if(IS_NETGAME && deathmatch && cfg.hudShown[HUD_FRAGS])
    {
        ...
        M_DrawText3(buf, HUDBORDERX, i, GF_FONTA, DTF_ALIGN_TOPLEFT|DTF_NO_EFFECTS);
    }
    ...
    if(cfg.hudShown[HUD_HEALTH])
    {
        ...
        M_DrawText3(buf, HUDBORDERX, h_height - HUDBORDERY - 4, GF_FONTA, DTF_ALIGN_BOTTOM|DTF_NO_EFFECTS);

        sprintf(buf, "%i", plr->health);
        DGL_Color4f(cfg.hudColor[0], cfg.hudColor[1], cfg.hudColor[2], textalpha);
        M_DrawText3(buf, HUDBORDERX + pos, h_height - HUDBORDERY, GF_FONTB, DTF_ALIGN_BOTTOM|DTF_NO_EFFECTS);
...
Draw_EndZoom();
    ...
    if(cfg.hudShown[HUD_AMMO])
    {
        ...
        for(ammotype=0; ammotype < NUM_AMMO_TYPES; ++ammotype)
        {
            ...
            pos = h_width/2;
            DGL_Color4f(cfg.hudColor[0], cfg.hudColor[1], cfg.hudColor[2], textalpha);
            M_DrawText3(buf, pos, h_height - HUDBORDERY, GF_FONTB, DTF_ALIGN_TOP|DTF_NO_EFFECTS);
            break;
        }
    }
    ...
    if(cfg.hudShown[HUD_ARMOR])
    {
        ...
        DGL_Color4f(1, 1, 1, iconalpha);
        M_DrawText3(buf, h_width - HUDBORDERX, h_height - HUDBORDERY - 4, GF_FONTA, DTF_ALIGN_BOTTOMRIGHT|DTF_NO_EFFECTS);

        sprintf(buf, "%i", plr->armorPoints);
        DGL_Color4f(cfg.hudColor[0], cfg.hudColor[1], cfg.hudColor[2], textalpha);
        M_DrawText3(buf, h_width - (w/2) - HUDBORDERX, h_height - HUDBORDERY, GF_FONTB, DTF_ALIGN_BOTTOMRIGHT|DTF_NO_EFFECTS);
    }
