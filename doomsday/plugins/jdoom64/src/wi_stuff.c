...
void WI_drawLF(void)
{
    ...
    // Draw <LevelName>
    WI_DrawPatch2(mapNamePatches[mapnum].id, SCREENWIDTH / 2, y, lname, false, ALIGN_CENTER);
...
void WI_drawEL(void)
{
    ...
    WI_DrawPatch2(mapNamePatches[(wbs->episode * 8) + wbs->nextMap].id, SCREENWIDTH / 2, y, lname, false, ALIGN_CENTER);
}
...
int WI_drawNum(int x, int y, int n, int digits)
{
    ...
    if(neg)
        WI_DrawPatch(minus.id, x -= 8, y, NULL, false);
