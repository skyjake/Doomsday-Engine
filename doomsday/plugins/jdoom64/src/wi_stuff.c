...
static void drawFinishedTitle(void)
{
    ...
    WI_DrawPatch2(patchId, Hu_ChoosePatchReplacement2(patchId, mapName, false), x, y, ALIGN_TOP, 0, 0);
    if(R_GetPatchInfo(patchId, &info))
        y += (5 * info.height) / 4;
    // Draw "Finished!"
    WI_DrawPatch2(pFinished, Hu_ChoosePatchReplacement(pFinished), x, y, ALIGN_TOP, 0, 0);
...
static void drawEnteringTitle(void)
{
    ...
    // Draw "Entering"
    WI_DrawPatch2(pEntering, Hu_ChoosePatchReplacement(pEntering), x, y, ALIGN_TOP, 0, 0);
...
static void drawTime(int x, int y, int t)
{
    ...
    if(!R_GetPatchInfo(pSucks, &info))
        return;
    WI_DrawPatch2(pSucks, Hu_ChoosePatchReplacement(pSucks), x - info.width, y, ALIGN_TOPLEFT, 0, DTF_NO_EFFECTS);
    }
}
