...
void P_CalcHeight(player_t *player)
{
    ...
    player->bob =
        FixedMul(player->plr->mo->momx,
                 player->plr->mo->momx) + FixedMul(player->plr->mo->momy,
                                                   player->plr->mo->momy);
    player->bob >>= 2;
    ...
    {
        player->plr->viewz =
            player->plr->mo->z + player->plr->viewheight - (20 * FRACUNIT);
    }
    else
    {
...
void P_MovePlayer(player_t *player)
{
    ...
    else
    {
        ...
        int     movemul = (onground || (player->plr->mo->flags2 & MF2_FLY))? 2048 :
        ...
        {
            if(player->plr->mo->state == &states[PCLASS_INFO(player->class)->normalstate])
            {
...
void P_ClientSideThink()
{
    ...
    if(!IS_CLIENT || !Get(DD_GAME_READY) || !mo)
        return;
