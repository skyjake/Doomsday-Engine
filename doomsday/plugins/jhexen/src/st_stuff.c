    ...
    int* drawnWidth, int* drawnHeight)
{
    ...
    if(plr->powers[PT_FLIGHT] > BLINKTHRESHOLD || !(plr->powers[PT_FLIGHT] & 16))
    {
        ...
        Hu_DrawPatch(dpSpinFly[frame].id, 16, 14, true);
    }
    *drawnWidth = 32;
    *drawnHeight = 28;
}
    ...
    int* drawnWidth, int* drawnHeight)
{
    ...
    {
        DGL_Color4f(1, 1, 1, iconAlpha);
        Hu_DrawPatch(dpSpinSpeed[(mapTime / 3) & 15].id, 12, 14, true);
    }
    *drawnWidth = 24;
    *drawnHeight = 28;
}
    ...
    int* drawnWidth, int* drawnHeight)
{
    ...
    {
        DGL_Color4f(1, 1, 1, iconAlpha);
        Hu_DrawPatch(dpSpinDefense[(mapTime / 3) & 15].id, -13, 14, true);
    }
    *drawnWidth = 26;
    *drawnHeight = 28;
}
    ...
    int* drawnWidth, int* drawnHeight)
{
    ...
    {
        DGL_Color4f(1, 1, 1, iconAlpha);
        Hu_DrawPatch(dpSpinMinotaur[(mapTime / 3) & 15].id, -13, 17, true);
    }
    *drawnWidth = 26;
    *drawnHeight = 29;
}
...
#define ORIGINY (-ST_HEIGHT*hud->showBar)
    ...
    {
        DGL_Color4f(1, 1, 1, iconAlpha);
        Hu_DrawPatch(dpWeaponFull[pClass].id, ORIGINX+190, ORIGINY, true);
    }
    else
    {
        ...
        {
            DGL_Color4f(1, 1, 1, iconAlpha);
            Hu_DrawPatch(dpWeaponPiece1[pClass].id, ORIGINX+PCLASS_INFO(pClass)->pieceX[0], ORIGINY, true);
        }

        if(plr->pieces & WPIECE2)
        {
            DGL_Color4f(1, 1, 1, iconAlpha);
            Hu_DrawPatch(dpWeaponPiece2[pClass].id, ORIGINX+PCLASS_INFO(pClass)->pieceX[1], ORIGINY, true);
        }

        if(plr->pieces & WPIECE3)
        {
            DGL_Color4f(1, 1, 1, iconAlpha);
            Hu_DrawPatch(dpWeaponPiece3[pClass].id, ORIGINX+PCLASS_INFO(pClass)->pieceX[2], ORIGINY, true);
        }
    }
...
#define ORIGINY (-HEIGHT * hud->showBar)
    ...
    {
        DGL_Color4f(1, 1, 1, 1);
        Hu_DrawPatch(dpStatusBar.id, ORIGINX, ORIGINY-28, true);
        /**
        ...
        //// \kludge end
        DGL_Color4f(1, 1, 1, 1);
        Hu_DrawPatch(dpStatusBarTop.id, ORIGINX, ORIGINY-28, true);
        ...
        if(!Hu_InventoryIsOpen(player))
        {
            ...
            {
                Hu_DrawPatch(dpStatBar.id, ORIGINX+38, ORIGINY, true);

                if(deathmatch)
                {
                    Hu_DrawPatch(dpKills.id, ORIGINX+38, ORIGINY, true);
                }

                Hu_DrawPatch(dpWeaponSlot[pClass].id, ORIGINX+190, ORIGINY, true);
            }
            else
            {
                Hu_DrawPatch(dpKeyBar.id, ORIGINX+38, ORIGINY, true);
            }
        }
        else
        {
            Hu_DrawPatch(dpInventoryBar.id, ORIGINX+38, ORIGINY, true);
        }
    }
    else
    {
...
#define ORIGINY (-ST_HEIGHT*hud->showBar)
    ...
    *drawnHeight = 0;
    ...
    for(i = 0; i < NUM_KEY_TYPES; ++i)
    {
        ...
        patch = &dpKeySlot[i];
        DGL_Color4f(1, 1, 1, iconAlpha);
        Hu_DrawPatch(patch->id, ORIGINX + 46 + numDrawn * 20, ORIGINY + 1, true);
...
#define ORIGINY (-ST_HEIGHT*hud->showBar)
    ...
    *drawnHeight = 0;
    ...
    for(i = 0; i < NUMARMOR; ++i)
    {
        ...
        Hu_DrawPatch(patch->id, ORIGINX + 150 + 31 * i, ORIGINY + 2, true);
...
#define ORIGINY (-ST_HEIGHT)
    ...
    DGL_Color4f(1, 1, 1, iconAlpha);
    Hu_DrawPatch(patch, ORIGINX+x, ORIGINY+y, true);
                      ...
                      float b, float a)
{
    ...
    if(val < 0)
    {
        ...
        if(val > 9)
        {
            Hu_DrawPatch(dpINumbers[val / 10].id, x + 8, y, true);
            Hu_DrawPatch(dpNegative.id, x, y, true);
        }
        else
        {
            Hu_DrawPatch(dpNegative.id, x + 8, y, true);
        }
        val = val % 10;
        Hu_DrawPatch(dpINumbers[val].id, x + 16, y, true);
        return;
    }
    if(val > 99)
    {
        Hu_DrawPatch(dpINumbers[val / 100].id, x, y, true);
    }
    val = val % 100;
    if(val > 9 || oldval > 99)
    {
    ...
    int* drawnWidth, int* drawnHeight)
{
    ...
    DGL_Color4f(1, 1, 1, iconAlpha);
    Hu_DrawPatch(patch->id, 0, 0, true);
    ...
    int* drawnWidth, int* drawnHeight)
{
    ...
    if(hud->currentInvItemFlash > 0)
    {
        ...
        Hu_DrawPatch(dpInvItemBox.id, -30, -30, true);

        DGL_Color4f(1, 1, 1, iconAlpha);
        Hu_DrawPatch(dp->id, -27, -30, true);
    }
    else
    {
