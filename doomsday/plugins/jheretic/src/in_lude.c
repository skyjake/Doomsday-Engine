...
void IN_DrawDMStats(void)
{
    ...
    if(interTime < 20)
    {
        for(i = 0; i < NUMTEAMS; ++i)
        {
            if(teamInfo[i].members)
            {
                DGL_DrawShadowedPatch(dpFaceAlive[i].id, 40, ((ypos << FRACBITS) + dSlideY[i] * interTime) >> FRACBITS);
                DGL_DrawShadowedPatch(dpFaceDead[i].id, ((xpos << FRACBITS) + dSlideX[i] * interTime) >> FRACBITS, 18);
            }
        }
    ...
    for(i = 0; i < NUMTEAMS; ++i)
    {
        if(teamInfo[i].members)
        {
            ...
            {
                DGL_DrawShadowedPatch(dpFaceAlive[i].id, 40, ypos);
                DGL_DrawShadowedPatch(dpFaceDead[i].id, xpos, 18);
            }
            else
            {
