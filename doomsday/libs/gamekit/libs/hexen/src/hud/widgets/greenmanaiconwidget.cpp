...
static patchid_t pGreenManaIcon[2];  ///< [ dim, bright ]
...
void GreenManaIconWidget_Draw(guidata_greenmanaicon_t *icon, const Point2Raw *offset)
{
    ...
    GL_DrawPatch(::pGreenManaIcon[icon->_iconIdx]);
...
#define Y_OFFSET            (   2 )
    ...
    DGL_Enable(DGL_TEXTURE_2D);
    DGL_Color4f(1, 1, 1, iconOpacity);
    GL_DrawPatch(::pGreenManaIcon[icon->_iconIdx], origin + Vec2i(X_OFFSET, Y_OFFSET));
    DGL_Disable(DGL_TEXTURE_2D);
...
void GreenManaIconWidget_UpdateGeometry(guidata_greenmanaicon_t *icon)
{
    ...
    patchinfo_t info;
    if(R_GetPatchInfo(::pGreenManaIcon[icon->_iconIdx], &info))
    {
...
{
    ::pGreenManaIcon[0] = R_DeclarePatch("MANADIM2");
    ::pGreenManaIcon[1] = R_DeclarePatch("MANABRT2");
}
