...
{
    fdata_t    *data = (fdata_t*) fd->finddata;
    char       *fn, *last;
    char        ext[256];
    struct stat st;
    ...
    if(fd->name)
        free(fd->name);
    fd->name = (char*) malloc(strlen(fn) + 1);
...
int myfindfirst(const char *filename, finddata_t *fd)
{
    ...
    fd->finddata = data = (fdata_t*) calloc(1, sizeof(fdata_t));
    fd->name = NULL;

    // Make a copy of the pattern.
    data->pattern = (char*) malloc(strlen(filename) + 1);
