...
{
    xgClassLinks = (xgclass_t *) game_GetAddress(DD_XGFUNC_LINK);
    if(!xgClassLinks)
    {
...
acfnptr_t Def_GetActionPtr(const char* name)
{
    ...
    // action functions.
    actionlink_t*       link =
        (actionlink_t *) game_GetAddress(DD_ACTION_LINK);
...
void Def_Read(void)
{
    ...
    for(i = 0; i < defs.count.materials.num; ++i)
    {
        ...
        if(def->layers[0].stageCount.num > 0)
        {
            ...
            {
                if(!(tex = GL_GetGLTextureByName(l->stages[0].name, (gltexture_type_t)l->stages[0].type)))
...
int Def_Get(int type, const char* id, void* out)
{
    ...
    case DD_DEF_LINE_TYPE:
        ...
        for(i = defs.count.lineTypes.num - 1; i >= 0; i--)
            ...
            {
                if(out)
                    Def_CopyLineType((linetype_t*)out, &defs.lineTypes[i]);
                return true;
            }
        }
        return false;
    ...
    case DD_DEF_SECTOR_TYPE:
        ...
        for(i = defs.count.sectorTypes.num - 1; i >= 0; i--)
            ...
            {
                if(out)
                    Def_CopySectorType((sectortype_t*)out, &defs.sectorTypes[i]);
                return true;
            }
        }
        return false;
...
int Def_Set(int type, int index, int value, void* ptr)
{
    ...
    case DD_DEF_SOUND:
        ...
        case DD_LUMP:
            S_StopSound(index, 0);
            strcpy(sounds[index].lumpName, (const char*) ptr);
    ...
    case DD_DEF_MUSIC:
        ...
        case DD_ID:
            if(ptr)
                strcpy(musdef->id, (const char*) ptr);
            break;

        case DD_LUMP:
            if(ptr)
                strcpy(musdef->lumpName, (const char*) ptr);
            break;
