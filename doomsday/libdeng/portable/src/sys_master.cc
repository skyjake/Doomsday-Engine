...
static serverlist_t *N_MasterNewServer(void)
{
    ...
    numServers++;
    node = (serverlist_t*) M_Calloc(sizeof(serverlist_t));
    node->next = servers;
    servers = node;
    return node;
}
...
{
    ddstring_t *msg = (ddstring_t*) stream;
...
{
	ddstring_t* response = (ddstring_t*) stream;
	size_t bytes = size * nmemb;
...
void N_MasterAnnounceServer(boolean isOpen)
{
    ...
    info = (serverinfo_t*) M_Calloc(sizeof(serverinfo_t));
