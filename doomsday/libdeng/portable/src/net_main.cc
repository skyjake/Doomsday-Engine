...
void Net_AllocArrays(void)
{
    ...
    for(i = 0; i < DDMAXPLAYERS; ++i)
    {
        ...
        // buffers.
        clients[i].ticCmds = (byte*) M_Calloc(BACKUPTICS * TICCMD_SIZE);
        // The last cmd that was executed is stored here.
        clients[i].lastCmd = (ticcmd_t*) M_Calloc(TICCMD_SIZE);
        clients[i].aggregateCmd = (ticcmd_t*) M_Calloc(TICCMD_SIZE);
        clients[i].runTime = -1;
    }
}
