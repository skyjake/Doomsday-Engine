...
{
    return (bmap_t*) Z_Calloc(sizeof(bmap_t), PU_MAPSTATIC, 0);
}
                            ...
                            subsector_t** ssecs)
{
    ...
    {
        bmap_t*             bmap = (bmap_t*) blockmap;
        ssecmapblock_t*     block = (ssecmapblock_t*)
...
typedef struct subsecmap_s {
    ...
    bmap = (ssecmap_t*) M_Calloc(sizeof(ssecmap_t) * subMapWidth * subMapHeight);
    ...
    for(i = 0; i < map->numSSectors; ++i)
    {
        ...
        for(x = xl; x <= xh; ++x)
            for(y = yl; y <= yh; ++y)
            {
                ...
                // Create a new node.
                iter = (ssecnode_t*) M_Malloc(sizeof(ssecnode_t));
                iter->data = ssec;
    ...
    ssecLinks = (subsector_t**) Z_Malloc(totalLinks * sizeof(subsector_t*), PU_MAP, NULL);
                                ...
                                void* data)
{
    ...
    {
        bmap_t*             bmap = (bmap_t*) blockmap;
        bmapblock_t*        bmapBlock = (bmapblock_t*)
                                     ...
                                     void* data)
{
    ...
    {
        bmap_t*             bmap = (bmap_t*) blockmap;
        ssecmapblock_t*     ssecBlock = (ssecmapblock_t*)
            M_GridmapGetBlock(bmap->gridmap, block[VX], block[VY], false);
...
{
    bmapblock_t*        block = (bmapblock_t*) blockPtr;
...
void rendBlockSubsectors(void* blockPtr, void* param,
                         float r, float g, float b, float a)
{
    ssecmapblock_t*     block = (ssecmapblock_t*) blockPtr;
...
static void drawBlockInfoBox(uint vBlock[2])
{
    ...
    block = (bmapblock_t*) M_GridmapGetBlock(bmap->gridmap, vBlock[VX], vBlock[VY], false);
    if(block)
    {
