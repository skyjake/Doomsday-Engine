...
static void buildSegsFromHEdges(gamemap_t* dest, binarytree_t* rootNode)
{
    ...
    index = (hedge_t**) M_Malloc(sizeof(hedge_t*) * params.curIdx);
    ...
    dest->segs = (seg_t*) Z_Calloc(dest->numSegs * sizeof(seg_t), PU_MAPSTATIC, 0);
                              ...
                              hedge_t* list, size_t segCount)
{
    ...
    segs = (seg_t**) Z_Malloc(sizeof(seg_t*) * (segCount + 1), PU_MAPSTATIC, 0);
...
static boolean C_DECL hardenNode(binarytree_t* tree, void* data)
{
    ...
    nodeData = (bspnodedata_t*) BinaryTree_GetData(tree);
