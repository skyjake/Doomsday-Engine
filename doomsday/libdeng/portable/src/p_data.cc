...
void P_RegisterUnknownTexture(const char *name, boolean planeTex)
{
    ...
    {   // A new unknown texture. Add it to the list
        if(++numBadTexNames > maxBadTexNames)
        {
            ...
            badTexNames = (badtex_t*) M_Realloc(badTexNames, sizeof(badtex_t)
                                                * maxBadTexNames);
        }

        badTexNames[numBadTexNames -1].name = (char*) M_Malloc(strlen(namet) +1);
                                    ...
                                    boolean canCreate)
{
    ...
    gameMapObjDefs = (gamemapobjdef_t*)
                              ...
                              uint elmIdx, boolean canCreate)
{
    ...
    {   // We haven't yet created the lists.
        moData->objLists = (gamemapobjlist_t*) M_Malloc(sizeof(*objList) * numGameMapObjDefs);
        for(i = 0; i < numGameMapObjDefs; ++i)
        {
    ...
    objList->objs = (gamemapobj_t**)
        M_Realloc(objList->objs, ++objList->num * sizeof(gamemapobj_t*));

    gmo = objList->objs[objList->num - 1] = (gamemapobj_t*) M_Malloc(sizeof(*gmo));
    gmo->elmIdx = elmIdx;
    gmo->numProps = 0;
    gmo->props = NULL;
