...
static __inline listnode_t *newNode(list_t *list)
{
    ...
    n->data = NULL;
    n->next = n->prev = (listnode_s*) n->data;
    return n;
}
...
static linklist_t *createList(int flags, comparefunc cFunc)
{
    ...
    liststate_t *state = (liststate_t*) malloc(sizeof(liststate_t));
...
static void testList(int cFlags)
{
    ...
    data = (int*) List_GetFront(list);
    checkError(list);
    if(!data || *data != 89)
        printf("ERROR: Incorrect first element!\n");

    data = (int*) List_GetBack(list);
    checkError(list);
    if(!data || *data != 2456)
    ...
    data = (int*) List_ExtractFront(list);
    checkError(list);
    if(!data || *data != 76889)
        printf("ERROR: Incorrect first element!\n");

    data = (int*) List_ExtractBack(list);
    checkError(list);
    if(!data || *data != -4005)
        printf("ERROR: Incorrect last element!\n");
    ...
    data = (int*) List_GetAt(list, 1);
    checkError(list);
    if(!data || *data != -45)
        printf("ERROR: Incorrect element #1!\n");

    data = (int*) List_GetAt(list, 7);
    checkError(list);
    if(!data || *data != 680)
