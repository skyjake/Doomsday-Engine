...
void GL_UploadTextureContent(texturecontent_t* content)
{
    ...
    else
    {
            ...
            content->format == DGL_DEPTH_COMPONENT))
        {
            ...
            allocatedTempBuffer = (byte*) M_Malloc(total * 2);
...
VERBOSE( Con_Message("GL_NewTexture: Uploading (%i:%ix%i) while not busy! "
    ...
    else
    {
        ...
        deferred_t* d = (deferred_t*) M_Calloc(sizeof(deferred_t));
...
DGLuint GL_NewTextureWithParams3(int format, int width,
                                 ...
                                 int anisoFilter, int wrapS, int wrapT)
{
    return GL_NewTextureWithParams2((dgltexformat_t)format, width, height, pixels, flags,
