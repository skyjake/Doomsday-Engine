...
void Con_SetString(const char* name, char* text, byte overRide)
{
    ...
    if(cvar->type == CVT_CHARPTR)
    {
        ...
        CV_CHARPTR(cvar) = (char*) M_Malloc(strlen(text) + 1);
...
void Con_AddVariable(cvar_t* var)
{
    ...
    if(++numCVars > maxCVars)
    {
        ...
        if(maxCVars < numCVars)
            maxCVars = numCVars;
        cvars = (cvar_t*) M_Realloc(cvars, sizeof(cvar_t) * maxCVars);
    }
    ...
    nameCopy = (char*) Z_Malloc(strlen(var->name) + 1, PU_STATIC, NULL);
  ...
  Con_Message("Con_AddCommand: CCmd \"%s\" will not have it's usage "
    ...
    {   // Allocate some more.
        ...
        if(maxCCmds < numCCmds)
            maxCCmds = numCCmds;
        ccmds = (ddccmd_t*) M_Realloc(ccmds, sizeof(ddccmd_t) * maxCCmds);
    }
...
calias_t* Con_AddAlias(const char* aName, const char* command)
{
    ...
    caliases = (calias_t*) M_Realloc(caliases, sizeof(calias_t) * (++numCAliases));
    cal = caliases + numCAliases - 1;

    // Allocate memory for them.
    cal->name = (char*) M_Malloc(strlen(aName) + 1);
    cal->command = (char*) M_Malloc(strlen(command) + 1);
...
knownword_t** Con_CollectKnownWordsMatchingWord(const char* word, uint* count)
{
    ...
    matches = (knownword_t**) M_Malloc(sizeof(knownword_t *) * (num + 1));
