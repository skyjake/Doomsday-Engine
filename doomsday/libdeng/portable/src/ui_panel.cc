...
void CP_CvarList(ui_object_t *ob)
{
    ...
    cvar_t     *var = Con_GetVariable((const char*) list->data);
...
void CP_CvarEdit(ui_object_t *ob)
{
    ...
    Con_SetString((const char*) ed->data, ed->ptr, true);
}

void CP_CvarSlider(ui_object_t *ob)
{
    uidata_slider_t *slid = (uidata_slider_t*) ob->data;
    cvar_t     *var = Con_GetVariable((const char*) slid->data);
...
void CP_VideoModeInfo(ui_object_t *ob)
{
    ...
    else
    {
        ...
        sprintf(buf, "%i x %i x %i (%s)", DD_WindowWidth(),
                DD_WindowHeight(), DD_WindowBPP(),
...
void CP_UpdateSetVidModeButton(int w, int h, int bpp, boolean fullscreen)
{
    ...
    if(w == DD_WindowWidth() && h == DD_WindowHeight() &&
       bpp == DD_WindowBPP() &&
...
void CP_InitCvarSliders(ui_object_t *ob)
{
    for(; ob->type; ob++)
    {
        if(ob->action == CP_CvarSlider)
        {
            ...
            if(slid->floatmode)
                slid->value = Con_GetFloat((const char*) slid->data);
            else
                slid->value = Con_GetInteger((const char*) slid->data);
        }
    }
}
...
D_CMD(OpenPanel)
{
    ...
    for(i = 0, ob = ob_panel; ob_panel[i].type; ++i, ob++)
    {
        ...
        else if(ob->action == CP_CvarList)
        {
            ...
            list->selection = UI_ListFindItem(ob, Con_GetInteger((const char*) list->data));
        }
        else if(ob->action == CP_CvarEdit)
        {
            uidata_edit_t *ed = (uidata_edit_t*) ob->data;

            strncpy(ed->ptr, Con_GetString((const char*) ed->data), ed->maxlen);
        }
    }
    ...
    // Update width the current resolution.
    {
        ...
        list = (uidata_list_t*) ob->data;
        list->selection = UI_ListFindItem(ob, RES(DD_WindowWidth(), DD_WindowHeight()));
        if(list->selection == -1)
        {
