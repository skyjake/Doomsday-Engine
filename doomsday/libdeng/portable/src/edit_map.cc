...
vertex_t* createVertex(void)
{
    ...
    vtx = (vertex_t*) M_Calloc(sizeof(*vtx));
    vtx->header.type = DMU_VERTEX;

    map->vertexes = (vertex_t**)
...
static linedef_t* createLine(void)
{
    ...
    line = (linedef_t*) M_Calloc(sizeof(*line));
    line->header.type = DMU_LINEDEF;

    map->lineDefs = (linedef_t**)
...
static sidedef_t* createSide(void)
{
    ...
    side = (sidedef_t*) M_Calloc(sizeof(*side));
    side->header.type = DMU_SIDEDEF;

    map->sideDefs = (sidedef_t**)
...
static sector_t* createSector(void)
{
    ...
    sec = (sector_t*) M_Calloc(sizeof(*sec));
    sec->header.type = DMU_SECTOR;

    map->sectors = (sector_t**) M_Realloc(map->sectors, sizeof(sec) * (++map->numSectors + 1));
...
static polyobj_t* createPolyobj(void)
{
    ...
    po = (polyobj_t*) M_Calloc(sizeof(*po));

    map->polyObjs = (polyobj_t**) M_Realloc(map->polyObjs, sizeof(po) * (++map->numPolyObjs + 1));
...
{
    const vertex_t     *a = *((const vertex_t **) p1);
    const vertex_t     *b = *((const vertex_t **) p2);
...
void MPE_DetectDuplicateVertices(editmap_t *map)
{
    ...
    hits = (vertex_t**) M_Malloc(map->numVertexes * sizeof(vertex_t*));
...
boolean MPE_RegisterUnclosedSectorNear(sector_t *sec, double x, double y)
{
    ...
    // A new one.
    unclosedSectors = (usecrecord_t*) M_Realloc(unclosedSectors,
...
static void hardenLinedefs(gamemap_t *dest, editmap_t *src)
{
    ...
    dest->lineDefs = (linedef_t*) Z_Calloc(dest->numLineDefs * sizeof(linedef_t), PU_MAPSTATIC, 0);
...
static void hardenSidedefs(gamemap_t* dest, editmap_t* src)
{
    ...
    dest->sideDefs = (sidedef_t*) Z_Malloc(dest->numSideDefs * sizeof(sidedef_t), PU_MAPSTATIC, 0);
...
static void hardenSectors(gamemap_t* dest, editmap_t* src)
{
    ...
    dest->sectors = (sector_t*) Z_Malloc(dest->numSectors * sizeof(sector_t), PU_MAPSTATIC, 0);
...
static void hardenPolyobjs(gamemap_t* dest, editmap_t* src)
{
    ...
    dest->polyObjs = (polyobj_t**) Z_Malloc((dest->numPolyObjs+1) * sizeof(polyobj_t*),
                              PU_MAP, 0);
    ...
    for(i = 0; i < dest->numPolyObjs; ++i)
    {
        ...
        destP = (polyobj_t*) Z_Calloc(POLYOBJ_SIZE, PU_MAP, 0);
        destP->idx = i;
        ...
        destP->originalPts = (fvertex_s*)
            Z_Malloc(destP->numSegs * sizeof(fvertex_t), PU_MAP, 0);
        destP->prevPts = (fvertex_s*)
            Z_Malloc(destP->numSegs * sizeof(fvertex_t), PU_MAP, 0);

        // Create a seg for each line of this polyobj.
        segs = (seg_t*) Z_Calloc(sizeof(seg_t) * destP->numSegs, PU_MAP, 0);
        destP->segs = (seg_s**) Z_Malloc(sizeof(seg_t*) * (destP->numSegs+1), PU_MAP, 0);
...
boolean MPE_End(void)
{
    ...
    gamemap = (gamemap_t*) Z_Calloc(sizeof(*gamemap), PU_MAPSTATIC, 0);
                     ...
                     float normalX, float normalY, float normalZ)
{
    ...
    pln = (plane_t*) M_Calloc(sizeof(plane_t));
    pln->height = height;
    ...
    newList = (plane_t**) M_Malloc(sizeof(plane_t*) * (++s->planeCount + 1));
                       ...
                       int sequenceType, float anchorX, float anchorY)
{
    ...
    po = createPolyobj();
    po->buildData.lineDefs = (linedef_t**) M_Calloc(sizeof(linedef_t*) * (lineCount+1));
    for(i = 0; i < lineCount; ++i)
    {
