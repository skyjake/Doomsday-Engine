...
static zipentry_t* newZipEntry(const char* name)
{
    ...
    zipFiles[oldCount].name = (char*) M_Malloc(sizeof(char) * (strlen(name) + 1));
...
static package_t* newPackage(void)
{
    ...
    package_t*          newPack = (package_t*) M_Calloc(sizeof(package_t));
...
boolean Zip_Open(const char* fileName, DFILE* prevOpened)
{
    ...
    pack = newPackage();
    pack->name = (char*) M_Malloc(sizeof(char) * (strlen(fileName) + 1));
        ...
        ++index, pos += sizeof(centralfileheader_t))
    {
        ...
        centralfileheader_t* header = (centralfileheader_t*) pos;
