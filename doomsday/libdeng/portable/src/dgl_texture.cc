...
#define SIZEOF18TO8     (sizeof(ushort) * 262144) // \fixme Too big?
        ...
        if(!pal->pal18To8)
        {
            pal->pal18To8 = (ushort*) Z_Malloc(SIZEOF18TO8, PU_STATIC, 0);
        }
                              ...
                              ushort num)
{
    ...
    colorPalettes = (gl_colorpalette_t*) Z_Realloc(colorPalettes,
        ++numColorPalettes * sizeof(*pal), PU_STATIC);

    pal = &colorPalettes[numColorPalettes-1];

    pal->num = num;
    pal->data = (DGLubyte*) Z_Malloc(pal->num * sizeof(DGLubyte) * 3, PU_STATIC, 0);
...
boolean grayMipmap(dgltexformat_t format, int width, int height, void *data)
{
    ...
    faded = (byte*) M_Malloc(size / 4);
    image = (byte*) M_Malloc(size);

    // Initial fading.
    if(format == DGL_LUMINANCE || format == DGL_RGB)
    {
        for(i = 0, in = (byte*) data, out = image; i < size; ++i, in += comps)
        {
                    ...
                    int width, int height, int genMips, void *data)
{
    ...
    {   // Use true color textures.
        ...
        {   // Needs converting.
            ...
            needFree = true;
            buffer = (byte*) M_Malloc(numPixels * 4);
            if(!buffer)
                return false;
