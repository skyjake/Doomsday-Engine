...
{
    listnode_t         *node = (listnode_t*) Z_Calloc(sizeof(listnode_t), PU_STATIC, 0);
    return node;
}
...
{
    maplumpinfo_t *info = (maplumpinfo_t*) Z_Calloc(sizeof(maplumpinfo_t), PU_STATIC, 0);
    return info;
}
...
static void freeMapLumpInfoList(listnode_t* headPtr)
{
    ...
    while(node)
    {
        ...
        if(node->data)
            freeMapLumpInfo((maplumpinfo_t*) node->data);
        freeListNode(node);
...
{
    archivedmap_t *dam = (archivedmap_t*) Z_Calloc(sizeof(archivedmap_t), PU_STATIC, 0);
    return dam;
}
                                        ...
                                        filename_t cachedMapDataFile)
{
    ...
    dam->lumpList = (int*) Z_Malloc(sizeof(int) * dam->numLumps, PU_STATIC, 0);
    ln = headPtr;
    i = 0;
    while(ln)
    {
...
{
    *map = (gamemap_t*) Z_Calloc(sizeof(gamemap_t), PU_MAPSTATIC, 0);
