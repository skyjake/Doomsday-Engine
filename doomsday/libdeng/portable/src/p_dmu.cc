...
void P_InitMapUpdate(void)
{
    ...
    usingDMUAPIver = game_GetInteger(DD_GAME_DMUAPI_VER);
    if(!usingDMUAPIver)
    ...
    dummyLines = (dummyline_t*) Z_Calloc(dummyCount * sizeof(dummyline_t), PU_STATIC, NULL);
    dummySectors = (dummysector_t*) Z_Calloc(dummyCount * sizeof(dummysector_t), PU_STATIC, NULL);
}
                  ...
                  uint index)
{
    ...
    else if(valueType == DDVT_BLENDMODE)
    {
        ...
        case DDVT_INT:
            ...
            *d = (blendmode_e) args->intValues[index];
            break;
        default:
...
/*
    ...
    {
    case DMU_SURFACE:
        Surface_SetProperty((surface_t*) obj, args);
        break;

    case DMU_PLANE:
        Plane_SetProperty((plane_t*) obj, args);
        break;

    case DMU_VERTEX:
        Vertex_SetProperty((vertex_t*) obj, args);
        break;

    case DMU_SEG:
        Seg_SetProperty((seg_t*) obj, args);
        break;

    case DMU_LINEDEF:
        Linedef_SetProperty((linedef_t*) obj, args);
        break;

    case DMU_SIDEDEF:
        Sidedef_SetProperty((sidedef_t*) obj, args);
        break;

    case DMU_SUBSECTOR:
        Subsector_SetProperty((subsector_t*) obj, args);
        break;

    case DMU_SECTOR:
        Sector_SetProperty((sector_t*) obj, args);
        break;
    ...
    if(updateSurface)
    {
        if(R_UpdateSurface(updateSurface, false))
        {
            ...
            {
            case DMU_SIDEDEF:
                updateSidedef = (sidedef_t*) updateSurface->owner;
                break;

            case DMU_PLANE:
                updatePlane = (plane_t*) updateSurface->owner;
                break;
                  ...
                  uint index)
{
    ...
    {
        const boolean* s = (const boolean*) src;
...
*/
    ...
    {
    case DMU_VERTEX:
        Vertex_GetProperty((const vertex_t*) obj, args);
        break;

    case DMU_SEG:
        Seg_GetProperty((const seg_t*) obj, args);
        break;

    case DMU_LINEDEF:
        Linedef_GetProperty((const linedef_t*) obj, args);
        break;

    case DMU_SURFACE:
        Surface_GetProperty((const surface_t*) obj, args);
        break;

    case DMU_PLANE:
        Plane_GetProperty((const plane_t*) obj, args);
        break;

    case DMU_SECTOR:
        Sector_GetProperty((const sector_t*) obj, args);
        break;

    case DMU_SIDEDEF:
        Sidedef_GetProperty((const sidedef_t*) obj, args);
        break;

    case DMU_SUBSECTOR:
        Subsector_GetProperty((const subsector_t*) obj, args);
        break;

    case DMU_MATERIAL:
        Material_GetProperty((const material_t*) obj, args);
        break;
