...
DGLuint GL_UploadTexture2(texturecontent_t* content)
{
    ...
    else
    {
        ...
        freeOriginal = true;
        rgbaOriginal = (byte*) M_Malloc(width * height * comps);
    ...
    {
        byte*           filtered = (byte*) M_Malloc(4 * width * height * 4);

        if(comps == 3)
        {
            // Must convert to RGBA.
            byte*           temp = (byte*) M_Malloc(4 * width * height);
    ...
    {
        freeBuffer = true;
        buffer = (byte*) M_Malloc(levelWidth * levelHeight * comps);
        if(noStretch)
        {
                          ...
                          void* context)
{
    ...
    {
        const byte*         data = (const byte*) W_CacheLumpNum(dTex->lump, PU_STATIC);

        // First try loading it as a PCX image.
        if(PCX_MemoryGetSize(data, &image->width, &image->height))
        {   // Nice...
            image->pixels = (byte*) M_Malloc(image->width * image->height * 3);
        ...
        else // It must be a raw image.
        {
            ...
            image->pixels = (byte*) M_Malloc(image->width * image->height);
                       ...
                       const char* name, gfxmode_t mode)
{
       ...
       GL_LoadImage(image, fileName))
    {
           ...
           image->height > GL_state.maxTexSize)
        {
            ...
            byte   *tmp = (byte*) M_Malloc(newWidth * newHeight * image->pixelSize);
                 ...
                 void* context)
{
    ...
    // Read in the flat.
    img->pixels = (byte*) M_Malloc(lumpLength);
...
static byte* loadImage(image_t* img, const char* imagefn)
{
    ...
    if(p)
    {
        if(!strcmp(p, "pcx"))
        {
            ...
            img->pixels = (byte*) M_Malloc(4 * img->width * img->height);
                             ...
                             int width, int height, boolean zeroMask)
{
    ...
    if(texDef->patchCount > 1)
    {
        ...
        imgdata = (byte*) M_Calloc(2 * numpels);
        ...
        for(i = 0; i < texDef->patchCount; ++i)
        {
            ...
            DrawRealPatch(imgdata, /*palette,*/ width, height,
                          (const lumppatch_t*) W_CacheLumpNum(patchDef->lump, PU_CACHE),
                          patchDef->offX, patchDef->offY,
                          zeroMask, false);
        }
    }
                           ...
                           void* context)
{
    ...
    else
    {
         ...
         */
        image->pixels = (byte*) M_Malloc(2 * image->width * image->height);
        image->isMasked =
...
byte GL_LoadDoomPatch(image_t* image, const patchtex_t* p)
{
    ...
    if(p->lump >= 0 && p->lump < numLumps)
    {
        ...
        patch = (const lumppatch_t*) W_CacheLumpNum(p->lump, PU_CACHE);
        ...
        if(patchWidth * patchHeight)
        {
            ...
            image->pixels = (byte*) M_Calloc(2 * image->width * image->height);
                   ...
                   void* context)
{
    ...
    {
        patch = (const lumppatch_t*) W_CacheLumpNum(sprTex->lump, PU_STATIC);
    }
...
byte GL_LoadRawTex(image_t* image, const rawtex_t* r)
{
    ...
    else
    {
        ...
        lumpdata = (const byte*) W_CacheLumpNum(r->lump, PU_STATIC);

        image->width = 320;

        // Try to load it as a PCX image first.
        image->pixels = (byte*) M_Malloc(3 * 320 * 200);
    ...
    if(!raw->tex)
    {
        ...
        else
        {
            ...
            if(!(image.height < 200))
            {
                ...
                // Two pieces:
                dat1 = (byte*) M_Calloc(comps * 256 * 256);
                dat2 = (byte*) M_Calloc(comps * 64 * 256);
...
DGLuint GL_GetFlareTexture(const char* name, int oldIdx)
{
    if(name && name[0])
    {
        ...
        if(name[0] >= '1' && name[0] <= '4' && !name[1])
            return GL_PrepareSysFlareTexture(flaretexid_t(name[0] - '1'));
...
{
    short*  dark = (short*) M_Calloc(width * height * 2);
    int     x, y;
    int     a, b;
    byte*   pix;
...
{
    int     x, y, c;
    byte*   result = (byte*) M_Calloc(4 * width * height);
...
DGLuint GL_PreparePatch2(patchtex_t* p)
{
    if(!p->tex)
    {
        ...
        else if(result == 1)
        {
            ...
            if(scaleSharp)
            {
                byte*               rgbaPixels = (byte*) M_Malloc(numpels * 4 * 2);
                byte*               upscaledPixels = (byte*) M_Malloc(numpels * 4 * 4);
            ...
            if(p->width > GL_state.maxTexSize)
            {
                ...
                tmpBuf = (byte*) M_Malloc(2 * image.height *
...
if(type < GLT_SYSTEM || !(type < NUM_GLTEXTURE_TYPES))
    ...
    tex = (gltexture_t*) Z_Malloc(sizeof(*tex), PU_STATIC, 0);
    tex->type = type;
    ...
    else
    {
           ...
           image.height > GL_state.maxTexSize)
        {
            if(image.pixelSize == 3 || image.pixelSize == 4)
            {
                int     newWidth = MIN_OF(image.width, GL_state.maxTexSize);
                int     newHeight = MIN_OF(image.height, GL_state.maxTexSize);
                byte   *temp = (byte*) M_Malloc(newWidth * newHeight * image.pixelSize);
...
VERBOSE( Con_Message("GLTexture_Prepare: Uploaded \"%s\" (%i) while not busy! "
        ...
        {   // We have a new instance.
            ...
            node = (gltexture_inst_node_t*) Z_Malloc(sizeof(*node), PU_STATIC, 0);
...
D_CMD(TranslateFont)
{
    ...
    for(i = 0; i < 256; ++i)
    {
        ...
        if((lump = W_CheckNumForName(name)) != -1)
        {
            ...
            patch = (lumppatch_t*) M_Malloc(size);
