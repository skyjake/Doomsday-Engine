...
static char* sdup(const char* str)
{
    ...
    newstr = (char*) M_Malloc(strlen(str) + 1);
    strcpy(newstr, str);
    return newstr;
}
...
static boolean DED_CheckCondition(const char* cond, boolean expected)
{
    ...
    else if(isalnum(cond[0]))
    {
        ...
        value = !stricmp(cond, game_GetString(DD_GAME_MODE));
    }
    ...
    while(ReadToken())
    {
        ...
        if(ISTOKEN("Material"))
        {
            ...
            if(prevMaterialDefIdx >= 0 && bCopyNext)
            {
                ...
                for(i = 0; i < DED_MAX_MATERIAL_LAYERS; ++i)
                {
                    ...
                    if(l->stages)
                    {
                        mat->layers[i].stages = (ded_material_layer_stage_t*)
        ...
        {   // A new texenv.
            ...
            for(;;)
            {
                ...
                if(ISLABEL("Texture") || ISLABEL("Flat") ||
                   ISLABEL("Sprite") || ISLABEL("System"))
                {   // A new material name.
                    mn = (ded_materialid_t*) DED_NewEntry((void**)&tenv->materials,
        ...
        if(ISTOKEN("Values")) // String Values
        {
            ...
            for(;;)
            {
                ...
                {
                    // Begin a new group.
                    rootStr = (char*) M_Realloc(rootStr, strlen(rootStr)
                        + strlen(label) + 2);
        ...
        if(ISTOKEN("Generator")) // Particle Generator
        {
            ...
            if(prevGenDefIdx >= 0 && bCopyNext)
            {
                ...
                {
                    gen->stages = (ded_ptcstage_t*) M_Malloc(sizeof(ded_ptcstage_t) *
                                           gen->stageCount.max);
...
int DED_ReadLump(ded_t* ded, lumpnum_t lump)
{
    ...
    if((lumpLength = W_LumpLength(lump)) > 0)
    {
        ...
        result = DED_ReadData(ded, (char*) buf, W_LumpSourceFile(lump));
