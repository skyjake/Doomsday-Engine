...
boolean M_CheckFileID(const char *path)
{
    ...
    if(numReadFiles > maxReadFiles)
    {
        ...
        readFiles = (file_identifier_t*) M_Realloc(readFiles, sizeof(readFiles[0]) * maxReadFiles);
    }
...
#define BSIZE 1024
        ...
        while(!lzEOF(file))
        {
            ...
            {
                byte   *newbuf = (byte*) Z_Malloc(length + count, PU_STATIC, 0);
            ...
            {
                byte   *newbuf = (byte*) M_Realloc(buf, length + count);
