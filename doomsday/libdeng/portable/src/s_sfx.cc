...
void Sfx_ChannelUpdate(sfxchannel_t* ch)
{
    ...
    if(ch->emitter)
    {
        ...
        if(dynamic_cast<mobj_s*>(ch->emitter)) // P_IsMobjThinker(ch->emitter->thinker.function))
        {
    ...
    if(buf->flags & SFXBF_3D)
    {
        ...
        if(ch->emitter && ch->emitter != listener &&
           dynamic_cast<mobj_s*>(ch->emitter))
        {
...
void Sfx_InitChannels(void)
{
    ...
    channels = (sfxchannel_t*) Z_Calloc(sizeof(*channels) * numChannels, PU_STATIC, 0);
