...
{
    helpnode_t*         n = (helpnode_t*) M_Calloc(sizeof(*n));
...
static int DH_ReadStrings(char* fileName)
{
    ...
    while(!deof(file))
    {
        ...
        else if(node && (end = strchr(ptr, '='))) // It must be a key?
        {
            ...
            length = 0, hst->text = (char*) M_Malloc(0x10000);
            while(*ptr)
            {
                // Backslash escapes.
                if(*ptr == '\\')
            ...
            hst->text[length] = 0;
            hst->text = (char*) M_Realloc(hst->text, length + 1);
        }
    }
...
{
    helpnode_t*         n = (helpnode_t*) foundNode;
    int                 i;
