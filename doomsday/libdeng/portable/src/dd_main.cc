...
void DD_AddIWAD(const char* path)
{
    ...
    iwadList[i] = (char*) M_Calloc(strlen(buf) + 1);   // This mem is not freed?
...
static void AddToWadList(char* list)
{
    ...
    char           *buffer = (char*) M_Malloc(len + 1), *token;
...
{
    autoload_t*         data = (autoload_t*) ptr;
...
void DD_AddStartupWAD(const char* file)
{
    ...
    _new = (char*) M_Calloc(strlen(temp) + 1);  // This is never freed?
    strcat(_new, temp);
    wadFiles[i] = _new;
}
