...
#define DO_LINKS(type, it, end) for(it = linkstore; it < end; it++) \
                                if(!func((type*)*it, data)) return false;
...
{
    linelinker_data_t*  data = (linelinker_data_t*) parm;
    nodeindex_t         nix;
...
void P_MobjLinkToRing(mobj_t* mo, linkmobj_t** link)
{
    ...
    {   // Create a new link at the current block cell.
        *link = (linkmobj_t*) Z_Malloc(sizeof(linkmobj_t), PU_MAP, 0);
        (*link)->next = NULL;
        (*link)->prev = NULL;
        (*link)->mobj = mo;
        return;
...
boolean PIT_AddLineIntercepts(linedef_t* ld, void* data)
{
    ...
    P_AddIntercept(frac, intercepttype_t(true), ld);
...
boolean PIT_AddMobjIntercepts(mobj_t* mo, void* data)
{
    ...
    P_AddIntercept(frac, intercepttype_t(false), mo);
