...
void C_RoverAdd(rover_t *r, void *node)
{
    ...
    if(!r->last)
        r->last = (rovernode_t*) node;
    if(r->first)
        r->first->prev = (rovernode_t*) node;

    ((rovernode_t*) node)->next = r->first;
    ((rovernode_t*) node)->prev = NULL;
    r->first = (rovernode_t*) node;
}
...
static clipnode_t *C_NewRange(binangle_t stAng, binangle_t endAng)
{
    ...
    if((node = (clipnode_t*) C_RoverGet(&clipNodes)) == NULL)
    {
        // Allocate a new node and add it to head the list.
        node = (clipnode_t*) Z_Malloc(sizeof(clipnode_t), PU_STATIC, NULL);
                               ...
                               float *normal, boolean topHalf)
{
    ...
    if((node = (occnode_t*) C_RoverGet(&occNodes)) == NULL)
    {
        // Allocate a new node.
        node = (occnode_t*) Z_Malloc(sizeof(occnode_t), PU_STATIC, NULL);
