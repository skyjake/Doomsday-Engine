...
#define MSFACTORS 7
    ...
    ssamples = (sector_t**) M_Malloc(sizeof(sector_t*) *
...
*/
    ...
    indexBitfield = (uint*) M_Calloc(bitfieldSize);
    contributorBitfield = (uint*) M_Calloc(bitfieldSize);
    ...
    blkSampleSectors = (sector_t**) M_Malloc(sizeof(sector_t*) * numSamples);
    if(numSamples > 1)
...
Con_Message("  Sector %i: %i / %i\n", s, changedCount, count);
        ...
        {
            sector->blocks = (ushort*) Z_Malloc(sizeof(unsigned short) * sector->blockCount,
                                      PU_MAPSTATIC, 0);
