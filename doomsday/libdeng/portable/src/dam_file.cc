...
static boolean openMapFile(char* path, boolean write)
{
    mapFile = NULL;
    mapFileVersion = 0;
    mapFile = lzOpen(path, (char*) (write? F_WRITE_PACKED : F_READ_PACKED));
...
static void readSide(const gamemap_t *map, uint idx)
{
    ...
    s->segs = (seg_t**) Z_Malloc(sizeof(seg_t*) * (s->segCount + 1), PU_MAP, 0);
    for(i = 0; i < s->segCount; ++i)
...
static void readSector(const gamemap_t *map, uint idx)
{
    ...
    s->blocks = (ushort*) Z_Malloc(sizeof(short) * s->blockCount, PU_MAP, 0);
    for(i = 0; i < s->blockCount; ++i)
        s->blocks[i] = readShort();

    // Line list.
    s->lineDefCount = (uint) readLong();
    s->lineDefs = (linedef_s**) Z_Malloc(sizeof(linedef_t*) * (s->lineDefCount + 1), PU_MAP, 0);
    for(i = 0; i < s->lineDefCount; ++i)
...
static void readPolyobj(const gamemap_t *map, uint idx)
{
    ...
    p->segs = (seg_t**) Z_Malloc(sizeof(seg_t*) * (p->numSegs + 1), PU_MAP, 0);
    for(i = 0; i < p->numSegs; ++i)
    {
        seg_t              *s = (seg_t*)
                            ...
                            boolean write)
{
    ...
    materialDict = (materialdict_t*) M_Calloc(sizeof(*materialDict));
