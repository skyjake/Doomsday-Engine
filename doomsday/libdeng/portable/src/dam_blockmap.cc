                         ...
                         uint blockno, linedef_t *line)
{
    ...
    l = (linelist_t*) M_Malloc(sizeof(linelist_t));
                              ...
                              linedef_t** lineDefs, uint* numLineDefs)
{
    ...
    blocklists = (linelist_t**) M_Calloc(numBlocks * sizeof(linelist_t*));
    blockcount = (uint*) M_Calloc(numBlocks * sizeof(uint));
    blockdone  = (uint*) M_Malloc(numBlocks * sizeof(uint));
    ...
    for(y = 0; y < bMapHeight; ++y)
        for(x = 0; x < bMapWidth; ++x)
        {
            ...
            if(count > 0)
            {
                ...
                lines = (linedef_t**) Z_Malloc((count + 1) * sizeof(linedef_t *),
                    PU_MAPSTATIC, NULL);
