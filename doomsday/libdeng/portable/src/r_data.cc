...
#define MAX_BPC         (16) // Max bits per component.
    ...
    {   // We are creating a new palette.
        if(!numColorPalettes)
        {
            colorPalettes = (colorpalettebind_t*) Z_Malloc(
                ++numColorPalettes * sizeof(colorpalettebind_t), PU_STATIC, 0);

            defaultColorPalette = numColorPalettes;
        }
        else
            colorPalettes = (colorpalettebind_t*) Z_Realloc(&colorPalettes,
...
patchtex_t** R_CollectPatchTexs(int* count)
{
    ...
    list = (patchtex_t**) Z_Malloc(sizeof(p) * (num + 1), PU_STATIC, NULL);
...
patchtex_t* R_GetPatchTex(lumpnum_t lump)
{
    ...
    p = (patchtex_t*) Z_Calloc(sizeof(*p), PU_PATCH, NULL);
...
rawtex_t** R_CollectRawTexs(int* count)
{
    ...
    // Allocate the array, plus one for the terminator.
    list = (rawtex_t**) Z_Malloc(sizeof(r) * (num + 1), PU_STATIC, NULL);
...
rawtex_t* R_GetRawTex(lumpnum_t lump)
{
    ...
    r = (rawtex_t*) Z_Calloc(sizeof(*r), PU_PATCH, NULL); // \todo Need a new cache tag?
...
static lumpnum_t* loadPatchList(lumpnum_t lump, size_t* num)
{
    ...
    patchLumpList = (lumpnum_t*) M_Malloc(numPatches * sizeof(*patchLumpList));
...
#pragma pack()
    ...
    validTexDefs = (byte*) M_Calloc(numTexDefs * sizeof(byte));
    texDefNumPatches = (short*) M_Calloc(numTexDefs * sizeof(*texDefNumPatches));
    ...
    if(numValidTexDefs > 0 && numValidPatchRefs > 0)
    {
        ...
        texDefs = (doomtexturedef_t**) Z_Malloc(numValidTexDefs * sizeof(*texDefs), PU_REFRESHTEX, 0);
        ...
        for(i = 0; i < numTexDefs; ++i, directory++)
        {
            ...
            {   // DOOM format.
                ...
                texDef = (doomtexturedef_t*) storage;
...
static void loadDoomTextureDefs(void)
{
    ...
    for(i = 0; i < numLumps; ++i)
    {
        ...
        if(!strncmp(name, "TEXTURE1", 8) || !strncmp(name, "TEXTURE2", 8))
        {
            ...
            if(*eList)
            {
                ...
                newList = (doomtexturedef_t**) Z_Malloc(sizeof(*newList) * ((*eCount) + newNumTexDefs),
                                   PU_REFRESHTEX, 0);
                n = 0;
                for(i = 0; i < *eCount; ++i)
                    newList[n++] = (*eList)[i];
    ...
    {   // There are custom doomtexturedefs, cross compare with the IWAD
        ...
        newList = (doomtexturedef_t**) Z_Malloc(sizeof(*newList) * (count + countCustom),
                           PU_REFRESHTEX, 0);
        n = 0;
        for(i = 0; i < count; ++i)
            newList[n++] = list[i];
...
static int R_NewFlat(lumpnum_t lump)
{
    ...
    newlist = (flat_t**) Z_Malloc(sizeof(flat_t*) * ++numFlats, PU_REFRESHTEX, 0);
    if(numFlats > 1)
    {
    ...
    flats = newlist;
    ptr = flats[numFlats - 1] = (flat_t*) Z_Calloc(sizeof(flat_t), PU_REFRESHTEX, 0);
    ptr->lump = lump;
...
Con_Message("R_GetSkinTex: Too many model skins!\n");
    ...
    skinNames = (skinname_t*) M_Realloc(skinNames, sizeof(skinname_t) * ++numSkinNames);
...
void R_InitTranslationTables(void)
{
    ...
    translationTables = (byte*) Z_Calloc(256 * 3 * 7, PU_REFRESHTRANS, 0);
}
...
void R_PrecacheMap(void)
{
    ...
    matPresent = (material_t**) M_Calloc(sizeof(material_t*) * numMaterials);
    n = 0;
...
Con_Message("R_CreateDetailTexture: Too many detail textures!\n");
    ...
    dTex = (detailtex_t*) M_Malloc(sizeof(*dTex));
    dTex->id = glTex->id;
    dTex->lump = lump;
    dTex->external = external;

    // Add it to the list.
    detailTextures = (detailtex_t**)
...
Con_Message("R_CreateLightMap: Too many lightmaps!\n");
    ...
    lmap = (lightmap_t*) M_Malloc(sizeof(*lmap));
    lmap->id = glTex->id;
    lmap->external = def->id;

    // Add it to the list.
    lightMaps = (lightmap_t**) M_Realloc(lightMaps, sizeof(lightmap_t*) * ++numLightMaps);
...
Con_Message("R_CreateFlareTexture: Too many flare textures!\n");
    ...
    fTex = (flaretex_t*) M_Malloc(sizeof(*fTex));
    fTex->external = def->id;
    fTex->id = glTex->id;

    // Add it to the list.
    flareTextures = (flaretex_t**)
...
Con_Message("R_CreateShinyTexture: Too many shiny textures!\n");
    ...
    sTex = (shinytex_t*) M_Malloc(sizeof(*sTex));
    sTex->id = glTex->id;
    sTex->external = def->shinyMap.path;

    // Add it to the list.
    shinyTextures = (shinytex_t**)
...
Con_Message("R_CreateMaskTexture: Too many mask textures!\n");
    ...
    mTex = (masktex_t*) M_Malloc(sizeof(*mTex));
    mTex->id = glTex->id;
