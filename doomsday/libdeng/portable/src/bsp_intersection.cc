...
{
    return (cnode_t*) M_Malloc(sizeof(cnode_t));
}
...
{
    return (clist_t*) M_Malloc(sizeof(clist_t));
}
...
{
    return (intersection_t*) M_Calloc(sizeof(intersection_t));
}
...
static void emptyCList(clist_t* list)
{
    ...
    while(node)
    {
        ...
        BSP_IntersectionDestroy((intersection_t*) node->data);
...
void BSP_InitIntersectionAllocator(void)
{
    if(!initedOK)
    {
        UnusedIntersectionList = (clist_t*) M_Calloc(sizeof(clist_t));
        unusedCNodes = NULL;
        initedOK = true;
    }
}
...
void BSP_ShutdownIntersectionAllocator(void)
{
    if(UnusedIntersectionList)
    {
        ...
        while(node)
        {
            ...
            freeIntersection((intersection_t*) node->data);
            freeCNode(node);
...
intersection_t* BSP_CutListFindIntersection(cutlist_t* cutList, vertex_t* v)
{
    ...
    while(node)
    {
        intersection_t *cut = (intersection_t*) node->data;
...
*/
    ...
    while(node && np)
    {
        intersection_t*     cur = (intersection_t*) node->data;
        intersection_t*     next = (intersection_t*) np->data;
...
*/
    ...
    {
        intersection_t*     cur = (intersection_t*) node->data;
        intersection_t*     next = (node->next? (intersection_t*) node->next->data : NULL);
