...
static objcontact_t* allocObjContact(void)
{
    ...
    if(contCursor == NULL)
    {
        con = (objcontact_t*) Z_Malloc(sizeof(*con), PU_STATIC, NULL);
...
static objlink_t* allocObjLink(void)
{
    ...
    {
        oLink = (objlink_t*) Z_Malloc(sizeof(*oLink), PU_STATIC, NULL);
                                   ...
                                   int height)
{
    ...
    obm = (objblockmap_t*) Z_Malloc(sizeof(objblockmap_t), PU_MAPSTATIC, 0);
    ...
    obm->width = width;
    obm->height = height;
    obm->blocks = (objblock_t*) Z_Malloc(sizeof(*obm->blocks) * obm->width * obm->height,
                           PU_MAPSTATIC, 0);
