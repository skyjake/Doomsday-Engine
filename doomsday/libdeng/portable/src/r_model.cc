...
static int R_NewModelFor(void /*const char *filename*/)
{
    ...
    for(i = 0; i < MAX_MODELS; ++i)
        if(!modellist[i])
        {
            modellist[i] = (model_t*) M_Calloc(sizeof(model_t));
            return i;
        }
...
static void R_LoadModelDMD(DFILE *file, model_t *mo)
{
    ...
    while(LONG(chunk.type) != DMC_END)
    {
        ...
        default:
            ...
            temp = (char*) M_Malloc(LONG(chunk.length));
    ...
    temp = (char*) AllocAndLoad(file, inf->offsetFrames,
