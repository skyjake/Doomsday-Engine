...
void Sv_MRAdd(pool_t *pool, const mobjdelta_t *delta)
{
	...
	if(!mis)
	{
		...
		mis->next = NULL;
		mis->prev = hash->last;
		if(hash->last) hash->last->next = mis;
		hash->last = mis;
		if(!hash->first) hash->first = mis;
	}
...
void Sv_MRRemove(pool_t *pool, thid_t id)
{
	...
	for(mis = hash->first; mis; mis = mis->next)
	{
		if(mis->id == id)
		{
			...
			if(hash->first == mis) hash->first = mis->next;
			if(hash->last == mis) hash->last = mis->prev;
			if(mis->next) mis->next->prev = mis->prev;
			if(mis->prev) mis->prev->next = mis->next;
...
int Sv_MRCheck(pool_t *pool, const mobjdelta_t *mobj)
{
	...
	if(!(mobj->delta.flags & MDF_MOM_X)) exclude |= MDF_POS_X;
	if(!(mobj->delta.flags & MDF_MOM_Y)) exclude |= MDF_POS_Y;
	if(!(mobj->delta.flags & MDF_MOM_Z)) exclude |= MDF_POS_Z;
