...
void SetValueStr(const char *path, const char *id, char *str)
{
	...
	for(i=0; i<ded->count.values.num; i++)
		...
		{
			ded->values[i].text = (char*) DD_Realloc(ded->values[i].text,
				strlen(str) + 1);
	...
	ded->values[i].text = 0;
	strcpy(ded->values[i].text = DD_Realloc(ded->values[i].text, 
		strlen(str) + 1), str);
}
...
*/
	while ((result = GetLine ()) == 1) 
	{
		...
		else LPrintf(unknown_str, Line1, "Ammo", ammoNum);
		if(!theAmmo) continue;

		if(max != -1) SetValueInt("Player|Max ammo", theAmmo, max);
		if(per != -1) SetValueInt("Player|Clip ammo", theAmmo, per);
	}
...
int PatchText (int oldSize)
{
	...
	if (oldSize < 7) {	// Music names are never >6 chars (sure they are -jk)
	...
	for(i = 0; i < ded->count.text.num; i++) 
	{
		...
		if(!stricmp(ded->text[i].text, oldString))
		{
			...
			ded->text[i].text = (char*) DD_Realloc(ded->text[i].text, 
				strlen(s)+1);
	...
	if(verbose && !good) LPrintf ("   (Unmatched)\n");
...
int DoInclude (int dummy)
{
	...
	if(verbose) LPrintf ("Including %s\n", com_token);
	...
	if(verbose) LPrintf ("Done with include\n");
...
void ReadDehacked(char *filename)
{
	...
	// Read in the patch.
	if((file = fopen(filename, "rt")) == NULL) return; // Shouldn't happen.
	// How long is it?
	fseek(file, 0, SEEK_END);
	len = ftell(file) + 1;
