...
static void LerpColor(unsigned char *out, uint c1, uint c2, uint c3,
					  uint f1, uint f2, uint f3)
{
	int i, total = f1 + f2 + f3;

	for(i = 0; i < 4; i++)
	{
		out[i] = (f1*COMP(i,c1) + f2*COMP(i,c2) + f3*COMP(i,c3)) / total;
	}
}
...
{
	// Alpha-difference?
	if((COMP(3,dw1) > 0) != (COMP(3,dw2) > 0)) return true;
...
#define BPP 4
  ...
  for (j=0; j<Yres; j++)
  {
    if (j>0)      prevline = -Xres*BPP; else prevline = 0;
    if (j<Yres-1) nextline =  Xres*BPP; else nextline = 0;
