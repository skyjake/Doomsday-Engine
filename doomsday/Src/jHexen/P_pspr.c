...
void P_SetPsprite(player_t * player, int position, statenum_t stnum)
{
	...
	do
	{
		...
		/*if(state->misc[0])
		   { // Set coordinates.
		   psp->sx = state->misc[0]<<FRACBITS;
		   player->plr->psprites[0].state = DDPSP_OFFSET;
		   }
		   if(state->misc[1])
		   {
		   psp->sy = state->misc[1]<<FRACBITS;
...
void P_FireWeapon(player_t * player)
{
	...
	{
		attackState =
			player->refire ? WeaponInfo[player->readyweapon][player->class].
			holdatkstate : WeaponInfo[player->readyweapon][player->class].
			atkstate;
	}
...
void C_DECL A_CStaffCheck(player_t * player, pspdef_t * psp)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		if(linetarget)
		{
			...
			pmo->angle =
				R_PointToAngle2(pmo->x, pmo->y, linetarget->x, linetarget->y);
...
static void CHolySeekerMissile(mobj_t *actor, angle_t thresh, angle_t turnMax)
{
	...
	if(!(leveltime & 15) || actor->z > target->z + (target->height) ||
...
static void CHolyTailFollow(mobj_t *actor, fixed_t dist)
{
	...
	if(child)
	{
		...
		oldDistance =
			P_ApproxDistance(child->x - actor->x, child->y - actor->y);
		if(P_TryMove
		   (child, actor->x + FixedMul(dist, finecosine[an]),
			actor->y + FixedMul(dist, finesine[an])))
		{
			newDistance =
				P_ApproxDistance(child->x - actor->x,
			...
			{
				child->z =
					actor->z + FixedMul(FixedDiv(newDistance, oldDistance),
										child->z - actor->z);
			}
		}
