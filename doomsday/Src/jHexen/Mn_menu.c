...
void MN_Drawer(void)
{
	...
	{
		int     effTime =
			(MenuTime > menuDarkTicks) ? menuDarkTicks : MenuTime;
...
static void SCFullscreenMana(int option)
{
	...
	{
		P_SetMessage(&players[consoleplayer], "MANA SHOWN IN FULLSCREEN VIEW",
					 true);
	}
	else
	{
		P_SetMessage(&players[consoleplayer], "NO MANA IN FULLSCREEN VIEW",
					 true);
	}
	S_LocalSound(SFX_CHAT, NULL);
}
...
static void SCLinearRaw(int option)
{
	...
	{
		P_SetMessage(&players[consoleplayer],
					 "GRAPHICS SCREENS USE LINEAR INTERPOLATION", true);
	}
	else
	{
		P_SetMessage(&players[consoleplayer],
					 "GRAPHICS SCREENS AREN'T INTERPOLATED", true);
	}
	S_LocalSound(SFX_CHAT, NULL);
...
{
	P_SetMessage(&players[consoleplayer],
				 (cfg.translucentIceCorpse ^=
				  1) ? "FROZEN MONSTERS ARE NOW TRANSLUCENT" :
				 "FROZEN MONSTERS NOT TRANSLUCENT", true);
	S_LocalSound(SFX_CHAT, NULL);
}
...
static void SCDynLights(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 (*(int *) cvDL->ptr ^=
				  1) ? "DYNAMIC LIGHTS ENABLED" : "DYNAMIC LIGHTS DISABLED",
				 true);
	S_LocalSound(SFX_CHAT, NULL);
}
...
static void SCSpriteBlending(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 (*(int *) cv->ptr ^=
				  1) ? "ADDITIVE BLENDING FOR EXPLOSIONS" :
				 "NO SPRITE BLENDING", true);
	S_LocalSound(SFX_CHAT, NULL);
}
...
static void SCSpriteLight(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 (*(int *) cv->ptr ^=
				  1) ? "SPRITES LIT BY DYNAMIC LIGHTS" :
				 "SPRITES NOT LIGHT BY LIGHTS", true);
	S_LocalSound(SFX_CHAT, NULL);
}
...
static void DrawControlsMenu(void)
{
		...
		i++, item++)
	{
		...
		MN_DrTextAYellow_CS(prbuff, menu->x + 134,
							menu->y + i * menu->itemHeight);
	}
}
...
static void SCJoySensi(int option)
{
	if(option == RIGHT_DIR)
	{
		if(cfg.joySensitivity < 9)
			cfg.joySensitivity++;
	}
	else if(cfg.joySensitivity > 1)
		cfg.joySensitivity--;
}
...
static void SCSaveGame(int option)
{
	...
	if(IS_CLIENT || Get(DD_PLAYBACK) || gamestate != GS_LEVEL)
		return;
...
boolean MN_Responder(event_t *event)
{
	...
	if(MenuActive == false)
	{
		if(key == DDKEY_ESCAPE	/*|| gamestate == GS_DEMOSCREEN */
		   || FI_IsMenuTrigger(event) || Get(DD_PLAYBACK))	// && !democam.mode))
		{
			MN_ActivateMenu();
