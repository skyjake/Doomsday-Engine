...
boolean P_LookForPlayers(mobj_t *actor, boolean allaround)
{
	...
	for( ; ; actor->lastlook = (actor->lastlook+1)&3 )
	{
		...
		{ // Player is invisible
			if((P_ApproxDistance(player->plr->mo->x-actor->x,
				player->plr->mo->y-actor->y) > 2*MELEERANGE)
				&& P_ApproxDistance(player->plr->mo->momx, player->plr->mo->momy)
				< 5*FRACUNIT)
...
void C_DECL A_FiredAttack(mobj_t *actor)
{
	...
	if (mo) S_StartSound(SFX_FIRED_ATTACK, actor);
}
...
void C_DECL A_IceGuyMissileExplode(mobj_t *actor)
{
	...
	for(i = 0; i < 8; i++)
	{
		mo = P_SpawnMissileAngle(actor, MT_ICEGUY_FX2, i*ANG45,	(int) (-0.3*FRACUNIT));
		if(mo)
		{
			mo->target = actor->target;
		}
...
//
	...
	else
	{
		...
		actor->special2 = 0;
		actor->momx = actor->momy = 0;
		dist=P_ApproxDistance(actor->x - target->x,
								actor->y - target->y);
