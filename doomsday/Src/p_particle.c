...
void P_NewParticle(ptcgen_t * gen)
{
	...
	pt->stage = 0;
	if(M_FRandom() < def->alt_variance)
		pt->stage = def->alt_start;
	pt->tics =
	...
	pt->mov[VX] =
		gen->vector[VX] +
		FRACUNIT * (def->vec_variance * (M_FRandom() - M_FRandom()));
	pt->mov[VY] =
		gen->vector[VY] +
		FRACUNIT * (def->vec_variance * (M_FRandom() - M_FRandom()));
	pt->mov[VZ] =
	...
	else if(gen->sector)		// The source is a plane?
	{
		...
		{
			pt->pos[VZ] =
				gen->sector->floorheight + i + FixedMul(M_Random() << 8,
														gen->sector->
														ceilingheight -
														gen->sector->
														floorheight - 2 * i);
...
void P_CheckPtcPlanes(void)
{
	...
	for(i = 0; i < numsectors; i++)
	{
		...
		for(p = 0; p < 2; p++)
		{
			plane = p;
			def =
				P_GetPtcGenForFlat(plane ? sector->ceilingpic : sector->
								   floorpic);
			if(!def)
				continue;
