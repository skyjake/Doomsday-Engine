...
void XS_SetSectorType(struct sector_s *sec, int special)
{
	...
	if(XS_GetType(special))
	{
		...
		if(!sec->xg)
			sec->xg = Z_Malloc(sizeof(xgsector_t), PU_LEVEL, 0);
...
void XS_MoverStopped(xgplanemover_t * mover, boolean done)
{
	...
	if(done)
	{
		...
		{
			XL_ActivateLine(true, &mover->origin->xg->info, mover->origin, 0,
							&dummything);
		}
		if(mover->flags & PMF_DEACTIVATE_WHEN_DONE && mover->origin)
		{
			XL_ActivateLine(false, &mover->origin->xg->info, mover->origin, 0,
							&dummything);
		}
		// Remove this thinker.
					...
					int *height, int *pic, sector_t **planesector)
{
	...
	case SPREF_SECTOR_TAGGED_CEILING:
		if(!(iter = XS_FindTagged(sector->tag)))
			return false;
		break;
	...
	case SPREF_LINE_TAGGED_CEILING:
		if(!actline)
			return false;
		if(!(iter = XS_FindTagged(actline->tag)))
			return false;
		break;
	...
	if(ref == SPREF_MY_FLOOR)
	{
		...
		if(height)
			*height = actline->frontsector->floorheight;
		if(pic)
			*pic = actline->frontsector->floorpic;
		if(planesector)
	...
	if(ref == SPREF_MY_CEILING)
	{
		...
		if(height)
			*height = actline->frontsector->ceilingheight;
		if(pic)
	...
	{
		if(height)
			*height = sector->origceiling;
		if(pic)
			*pic = sector->ceilingpic;
		return true;
	}
					 ...
					 void *context)
{
	...
	if(!XS_GetPlane(line, sector, info->iparm[7], 0, 0, &flat, 0))
		flat = info->iparm[8];
	if(flat > 0)
		XS_ChangePlaneTexture(sector, ceiling, flat);
	...
	if(XL_TraversePlanes
	   (line, info->iparm[11], info->iparm[12], 0, &st,
	...
	st = info->iparm[14];
	if(XL_TraversePlanes
	   (line, info->iparm[13], info->iparm[14], 0, &st,
		XSTrav_HighestSectorType))
	{
...
{
	if(!sector->xg || !(sector->xg->info.flags & STF_GRAVITY))
...
{
	if(!sector->xg || !(sector->xg->info.flags & STF_FRICTION))
...
int CCmdMovePlane(int argc, char **argv)
{
	...
	mover->destination =
		units +
		(isOffset ? (isCeiling ? sector->ceilingheight : sector->floorheight) :
		 0);
