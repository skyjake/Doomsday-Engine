	...
	for(i = 0; i < count_texts.num; i++)
	{
		...
		for(k = i + 1; k < count_texts.num; k++)
			if(!strcmp(defs.text[i].id, defs.text[k].id) && texts[k].text)
			{
				...
				texts[i].text =
					realloc(texts[i].text, strlen(texts[k].text) + 1);
...
void Def_CopyLineType(linetype_t * l, ded_linetype_t * def)
{
	...
	{
		if(mappings[i].lclass != l->line_class)
			continue;
		for(k = 0; k < 20; k++)
		{
			a = mappings[i].map[k];
			...
			{
				l->iparm[n] =
					Friendly(R_CheckTextureNumForName(def->iparm_str[n]));
			}
			else if(a & MAP_FLAT)
			{
				if(def->iparm_str[n][0])
					l->iparm[n] =
						Friendly(W_CheckNumForName(def->iparm_str[n]));
			}
			else if(a & MAP_MUS)
			{
