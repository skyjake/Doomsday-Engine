...
static void M_LetterFlash(int x, int y, int w, int h, int bright, float red,
...
void M_WriteText3(int x, int y, const char *string, dpatch_t * font, float red,
				  float green, float blue, boolean doTypeIn, int initialCount)
{
	int     pass;
	int     w, h;
	const char *ch;
	...
	for(pass = 0; pass < 2; ++pass)
	{
		...
		if(red >= 0)
			gl.Color4f(red, green, blue, menu_alpha);
		...
		while(1)
		{
			...
			if(pass)
			{
				...
				if(flash > 0)
				{
					M_LetterFlash(cx, cy + yoff, w, h, true, (1 + 2 * red) / 3,
								  (1 + 2 * green) / 3, (1 + 2 * blue) / 3,
			...
			else if(cfg.menuShadow > 0)
			{
				...
				M_LetterFlash(cx, cy + yoff, w, h, false, 1, 1, 1,
							  (red <
							   ...
							   255.0f : menu_alpha) * cfg.menuShadow);
			}
...
void M_WriteMenuText(Menu_t * menu, int index, char *text)
{
	...
	M_WriteText2(menu->x + off, menu->y + menu->itemHeight * index, text,
				 menu->font, 1, 1, 1);
}
