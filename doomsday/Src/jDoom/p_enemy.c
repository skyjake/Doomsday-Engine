...
boolean P_CheckMeleeRange(mobj_t *actor)
{
	...
	//{
	dist =
		P_ApproxDistance(dist,
						 (pl->z + (pl->height >> 1)) - (actor->z +
														(actor->height >> 1)));
	//}
...
void P_NewChaseDir(mobj_t *actor)
{
	...
	if(actor->floorz - actor->dropoffz > FRACUNIT * 24 && actor->z <= actor->floorz && !(actor->flags & (MF_DROPOFF | MF_FLOAT)) && P_AvoidDropoff(actor))	// Move away from dropoff
	{
...
void C_DECL A_VileTarget(mobj_t *actor)
{
	...
	fog =
		P_SpawnMobj(actor->target->x, actor->target->x, actor->target->z,
					MT_FIRE);
