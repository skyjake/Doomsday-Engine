...
cvar_t gameCVars[] =
{
	...
	"i_JoyXAxis", OBSOLETE, CVT_INT, &cfg.joyaxis[0], 0, 3, "0=None, 1=Move, 2=Turn, 3=Strafe.", 
	"i_JoyYAxis", OBSOLETE, CVT_INT, &cfg.joyaxis[1], 0, 3, "0=None, 1=Move, 2=Turn, 3=Strafe.", 
	"i_JoyZAxis", OBSOLETE, CVT_INT, &cfg.joyaxis[2], 0, 3, "0=None, 1=Move, 2=Turn, 3=Strafe.", 
//	"i_JoyDeadZone", OBSOLETE, CVT_INT, &cfg.joydead, 10, 90, "Joystick dead zone, in percents.", 
	...
	"EchoMsg", OBSOLETE, CVT_BYTE, &cfg.echoMsg, 0, 1, "1=Echo all messages to the console.", 
	...
	"d_ViewSize", OBSOLETE|CVF_PROTECTED, CVT_INT, &screenblocks, 3, 11, "View window size (3-11).", 
	"d_sbSize", OBSOLETE|CVF_PROTECTED, CVT_INT, &cfg.sbarscale, 1, 20, "Status bar size (1-20).", 
	"MapAlpha", OBSOLETE, CVT_FLOAT, &cfg.automapAlpha, 0, 1, "Alpha level of the automap background.", 
	"flash_R", OBSOLETE, CVT_FLOAT, &cfg.flashcolor[0], 0, 1, "Menu selection flash color, red component.", 
	"flash_G", OBSOLETE, CVT_FLOAT, &cfg.flashcolor[1], 0, 1, "Menu selection flash color, green component.", 
	"flash_B", OBSOLETE, CVT_FLOAT, &cfg.flashcolor[2], 0, 1, "Menu selection flash color, blue component.", 
	"flash_Speed", OBSOLETE, CVT_INT, &cfg.flashspeed, 0, 50, "Menu selection flash speed.", 
	"TurningSkull", OBSOLETE, CVT_BYTE, &cfg.turningSkull, 0, 1, "1=Menu skull turns at slider items.", 
	"hud_Health", OBSOLETE, CVT_BYTE, &cfg.hudShown[HUD_HEALTH], 0, 1, "1=Show health in HUD.", 
	"hud_Armor", OBSOLETE, CVT_BYTE, &cfg.hudShown[HUD_ARMOR], 0, 1, "1=Show armor in HUD.", 
	"hud_Ammo", OBSOLETE, CVT_BYTE, &cfg.hudShown[HUD_AMMO], 0, 1, "1=Show ammo in HUD.", 
	"hud_Keys", OBSOLETE, CVT_BYTE, &cfg.hudShown[HUD_KEYS], 0, 1, "1=Show keys in HUD.", 
	"hud_Frags", OBSOLETE, CVT_BYTE, &cfg.hudShown[HUD_FRAGS], 0, 1, "1=Show deathmatch frags in HUD.", 
	"hud_Scale", OBSOLETE, CVT_FLOAT, &cfg.hudScale, 0.1f, 10, "Scaling for HUD info.", 
	...
	"hud_ShowAllFrags", OBSOLETE, CVT_BYTE, &hu_showallfrags, 0, 1, "Debug: HUD shows all frags of all players.", 
	...
	"s_ReverbVol", OBSOLETE, CVT_BYTE, &cfg.snd_ReverbFactor, 0, 100, "General reverb strength (0-100).", 
	"s_Custom", OBSOLETE, CVT_BYTE, &cfg.customMusic, 0, 1, "1=Enable custom (external) music files.", 
	"ReverbDebug", OBSOLETE|CVF_NO_ARCHIVE, CVT_BYTE, &cfg.reverbDebug, 0, 1, "1=Reverb debug information in the console.", 
	...
	"n_Slot", OBSOLETE, CVT_BYTE, &cfg.netSlot, 0, 6, "The savegame slot to start from. 0=none.", 
	"n_Jump", OBSOLETE, CVT_BYTE, &cfg.netJumping, 0, 1, "1=Allow jumping in multiplayer games.", 
	"Deathmatch", OBSOLETE, CVT_BYTE, &cfg.netDeathmatch, 0, 2, "Start multiplayers games as deathmatch.", 
	"NoCoopDamage", OBSOLETE, CVT_BYTE, &cfg.noCoopDamage, 0, 1, "1=Disable player-player damage in co-op games.", 
	"NoCoopWeapons", OBSOLETE, CVT_BYTE, &cfg.noCoopWeapons, 0, 1, "1=Disable multiplayer weapons during co-op games.", 
	"NoTeamDamage", OBSOLETE, CVT_BYTE, &cfg.noTeamDamage, 0, 1, "1=Disable team damage (player color = team).", 
	"n_Color", OBSOLETE, CVT_BYTE, &cfg.netColor, 0, 3, "Player color: 0=green, 1=gray, 2=brown, 3=red.", 
	"AllowJump", OBSOLETE, CVT_INT, &cfg.jumpEnabled, 0, 1, "1=Allow jumping.", 
	"FastMonsters", OBSOLETE, CVT_BYTE, &fastparm, 0, 1, "1=Fast monsters in non-demo single player.", 
	"ZClip", OBSOLETE, CVT_BYTE, &cfg.moveCheckZ, 0, 1, "1=Allow mobjs to move under/over each other.", 
	"JumpPower", OBSOLETE, CVT_FLOAT, &cfg.jumpPower, 0, 100, "Jump power.", 
	"AutoSwitch", OBSOLETE, CVT_BYTE, &cfg.weaponAutoSwitch, 0, 1, "1=Change weapon automatically when picking one up.", 
	"SecretMsg", OBSOLETE, CVT_INT, &cfg.secretMsg, 0, 1, "1=Announce the discovery of secret areas.", 
	"EyeHeight", OBSOLETE, CVT_INT, &cfg.plrViewHeight, 41, 54, "Player eye height. The original is 41.", 
	"MenuScale", OBSOLETE, CVT_FLOAT, &cfg.menuScale, .1f, 1, "Scaling for menus.", 
	"MenuEffects", OBSOLETE, CVT_INT, &cfg.menuEffects, 0, 2, "Disable menu effects: 1=type-in, 2=all.", 
	"CounterCheat", OBSOLETE, CVT_INT, &cfg.counterCheat, 0, 63, "Automap kills/items/secret counter bitfield.", 
	"LevelTitle", OBSOLETE, CVT_BYTE, &cfg.levelTitle, 0, 1, "1=Show level title and author in the beginning.", 
	"Menu_R", OBSOLETE, CVT_FLOAT, &cfg.menuColor[0], 0, 1, "Menu color red component.", 
	"Menu_G", OBSOLETE, CVT_FLOAT, &cfg.menuColor[1], 0, 1, "Menu color green component.", 
	"Menu_B", OBSOLETE, CVT_FLOAT, &cfg.menuColor[2], 0, 1, "Menu color blue component.", 
	"MenuFog", OBSOLETE, CVT_INT, &cfg.menuFog, 0, 1, "Menu fog mode: 0=blue vertical, 1=black smoke.", 
	"MsgCount", OBSOLETE, CVT_INT, &cfg.msgCount, 0, 8, "Number of HUD messages displayed at the same time.", 
	"MsgScale", OBSOLETE|CVF_NO_MAX, CVT_FLOAT, &cfg.msgScale, 0, 0, "Scaling factor for HUD messages.", 
	"MsgUptime", OBSOLETE|CVF_NO_MAX, CVT_INT, &cfg.msgUptime, 35, 0, "Number of tics to keep HUD messages on screen.", 
...
//===========================================================================
	...
	"msg-echo",			0,			CVT_BYTE,	&cfg.echoMsg,		0, 1,	"1=Echo all messages to the console.",
	...
	"view-size",		CVF_PROTECTED,	CVT_INT,	&screenblocks,	3, 11,	"View window size (3-11).",
	...
	"hud-keys",			0,			CVT_BYTE,	&cfg.hudShown[HUD_KEYS],	0, 1,	"1=Show keys in HUD.",
	"hud-frags",		0,			CVT_BYTE,	&cfg.hudShown[HUD_FRAGS],	0, 1,	"1=Show deathmatch frags in HUD.",
	"hud-scale",		0,			CVT_FLOAT,	&cfg.hudScale,				0.1f, 10, "Scaling for HUD info.",
	...
	"server-game-deathmatch",	0,			CVT_BYTE,	&cfg.netDeathmatch,	0, 2,	"Start multiplayers games as deathmatch.",
	"server-game-coop-nodamage", 0,			CVT_BYTE,	&cfg.noCoopDamage,	0, 1,	"1=Disable player-player damage in co-op games.",
	"server-game-coop-noweapons", 0,		CVT_BYTE,	&cfg.noCoopWeapons, 0, 1,	"1=Disable multiplayer weapons during co-op games.",
	"server-game-noteamdamage",	0,			CVT_BYTE,	&cfg.noTeamDamage,	0, 1,	"1=Disable team damage (player color = team).",
	"server-game-deathmatch-killmsg", 0,	CVT_BYTE,	&cfg.killMessages,	0, 1,	"1=Announce frags in deathmatch.",
	"server-game-nobfg",		0,			CVT_BYTE,	&cfg.noNetBFG,		0, 1,	"1=Disable BFG9000 in all netgames.",
	"server-game-coop-nothing", 0,			CVT_BYTE,	&cfg.noCoopAnything, 0, 1,	"1=Disable all multiplayer objects in co-op games.",
	"server-game-coop-respawn-items", 0,	CVT_BYTE,	&cfg.coopRespawnItems, 0, 1, "1=Respawn items in co-op games.",
	...
	"menu-effect",		0,			CVT_INT,	&cfg.menuEffects,	0, 2,	"Disable menu effects: 1=type-in, 2=all.",
	"map-cheat-counter",0,			CVT_INT,	&cfg.counterCheat,	0, 63,	"Automap kills/items/secret counter bitfield.",
	"hud-title",		0,			CVT_BYTE,	&cfg.levelTitle,	0, 1,	"1=Show level title and author in the beginning.",
	"hud-title-noidsoft", 0,		CVT_BYTE,	&cfg.hideAuthorIdSoft, 0, 1, "1=Don't show map author if it's \"id Software\".",
