...
//
// $Log$
// Revision 1.12  2004/05/28 17:16:40  skyjake
...
void P_ExplodeMissile (mobj_t* mo)
{
	...
	if(mo->flags & MF_MISSILE)
	{
		...
		if(mo->flags & MF_BRIGHTSHADOW) mo->flags &= ~MF_BRIGHTSHADOW;
...
void P_ZMovement (mobj_t* mo)
{
    ...
    if(mo->flags & MF_FLOAT && mo->target)
    {
		...
		if(!(mo->flags & MF_SKULLFLY) && !(mo->flags & MF_INFLOAT))
		{
			...
			delta = (mo->target->z + mo->target->height/2) -
				(mo->z + mo->height/2);
...
{
	if(mobj->ddflags & DDMF_REMOTE)	return;
...
void P_SpawnPlayer (mapthing_t* mthing, int pnum)
{
	...
	if(pnum < 0) pnum = 0;
	if(pnum >= MAXPLAYERS-1) pnum = MAXPLAYERS-1;
	...
	if(i > 0) mobj->flags |= i << MF_TRANSSHIFT;
...
void P_SpawnMapThing (mapthing_t* mthing)
{
	...
	// a netgame.
    if(!IS_NETGAME && (mthing->options & 16)) return;
	...
	if(IS_CLIENT) 
	{
		if(!(mobjinfo[i].flags & MF_LOCAL)) return;
	}

	if(i == Get(DD_NUMMOBJTYPES)) return;
	...
	if(IS_NETGAME
		&& (mthing->options & 16))	// multiplayer flag
	{
		// Cooperative weapons?
		if (cfg.noCoopWeapons 
			&& !deathmatch 
			&& i >= MT_CLIP				// ammo...
		...
		if(cfg.noCoopAnything && !deathmatch) return;
		// BFG disabled in netgames?
		if(cfg.noNetBFG && i == MT_MISC25) return;
	}
