...
void MN_Drawer(void)
{
	...
	if(MenuActive == false)
	{
		if(askforquit)
		{
			...
			MN_DrTextA_CS(QuitEndMsg[typeofask - 1],
						  160 - MN_TextAWidth(QuitEndMsg[typeofask - 1]) / 2,
						  80);
			if(typeofask == 3)
			{
				MN_DrTextA_CS(SlotText[quicksave - 1],
			...
			if(typeofask == 4)
			{
				MN_DrTextA_CS(SlotText[quickload - 1],
	...
	if(MenuActive || menuDark > 0)
	{
		...
		y = CurrentMenu->y +
			((CurrentItPos -
			  CurrentMenu->firstItem) * CurrentMenu->itemHeight) +
...
static void DrawHUDMenu(void)
{
	...
	MN_DrawMenuText(menu, 13,
					countnames[(cfg.
								counterCheat & 0x1) | ((cfg.
														counterCheat & 0x8) >>
													   2)]);
	MN_DrawMenuText(menu, 14,
					countnames[((cfg.counterCheat & 0x2) >> 1) | ((cfg.
																   counterCheat
																   & 0x10) >>
																  3)]);
	MN_DrawMenuText(menu, 15,
					countnames[((cfg.counterCheat & 0x4) >> 2) | ((cfg.
																   counterCheat
																   & 0x20) >>
																  4)]);
}
...
static boolean SCFullscreenArmor(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 cfg.
				 showFullscreenArmor ? "ARMOR SHOWN IN FULLSCREEN VIEW" :
...
static boolean SCFullscreenKeys(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 cfg.
				 showFullscreenArmor ? "KEYS SHOWN IN FULLSCREEN VIEW" :
...
static boolean SCFullscreenMana(int option)
{
	...
	{
		P_SetMessage(&players[consoleplayer], "AMMO SHOWN IN FULLSCREEN VIEW",
					 true);
	}
	else
	{
		P_SetMessage(&players[consoleplayer], "NO AMMO IN FULLSCREEN VIEW",
					 true);
	}
	S_LocalSound(sfx_chat, NULL);
	return true;
...
static boolean SCSaveGame(int option)
{
	...
	if(!usergame || IS_CLIENT || Get(DD_PLAYBACK) || gamestate != GS_LEVEL)
	{
...
static boolean SC3DSounds(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 (*(int *) cv3ds->ptr ^=
				  1) ? "3D SOUND MODE" : "2D SOUND MODE", true);
	S_LocalSound(sfx_chat, NULL);
	return true;
}
...
static boolean SCSfx16bit(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 (*(int *) cv->ptr ^=
				  1) ? "16 BIT INTERPOLATION" : "8 BIT INTERPOLATION", true);
	S_LocalSound(sfx_chat, NULL);
	return true;
}
#endif
...
static boolean SCAllowJump(int option)
{
	...
	P_SetMessage(&players[consoleplayer],
				 cfg.jumpEnabled ? "JUMPING ALLOWED" : "JUMPING NOT ALLOWED",
				 true);
	S_LocalSound(sfx_chat, NULL);
	return true;
}
...
static boolean SCJoyLook(int option)
{
	P_SetMessage(&players[consoleplayer],
				 (cfg.usejlook ^=
				  1) ? "JOYSTICK LOOK ON" : "JOYSTICK LOOK OFF", true);
	S_LocalSound(sfx_chat, NULL);
	return true;
}
...
static boolean SCPOVLook(int option)
{
	P_SetMessage(&players[consoleplayer],
				 (cfg.povLookAround ^=
				  1) ? "POV LOOK ON" : "POV LOOK OFF", true);
	S_LocalSound(sfx_chat, NULL);
	return true;
}
...
static boolean SCInverseJoyLook(int option)
{
	P_SetMessage(&players[consoleplayer],
				 (cfg.jlookInverseY ^=
				  1) ? "INVERSE JOYLOOK" : "NORMAL JOYLOOK", true);
	S_LocalSound(sfx_chat, NULL);
	return true;
}
...
boolean MN_Responder(event_t *event)
{
		...
		case 'y':
			if(askforquit)
			{
				...
				case 2:
					...
					break;
				case 3:
					P_SetMessage(&players[consoleplayer], "QUICKSAVING....",
								 false);
					FileMenuKeySteal = true;
					...
					return true;
				case 4:
					P_SetMessage(&players[consoleplayer], "QUICKLOADING....",
								 false);
	...
	if(MenuActive == false)
	{
		if(key == DDKEY_ESCAPE
		   ...
		   || FI_IsMenuTrigger(event) || Get(DD_PLAYBACK))
		{
			MN_ActivateMenu();
	...
	if(!FileMenuKeySteal)
	{
		int     firstVI = CurrentMenu->firstItem, lastVI =
			firstVI + CurrentMenu->numVisItems - 1;
