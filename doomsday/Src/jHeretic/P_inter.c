...
void P_SetMessage(player_t *player, char *message, boolean ultmsg)
{
	...
	if((ultimatemsg || !cfg.messageson) && !ultmsg)
	{
		return;
	}
	strcpy(messagebuf, message);
	player->message = messagebuf;
	...
	// Messages for the consoleplayer will be shown in the console. 
	if(cfg.echoMsg && plrNum == consoleplayer) 
		Con_FPrintf(CBLF_CYAN, "%s\n", message);
	
	// Servers are responsible for sending these messages to the clients.
	NetSv_SendMessage(plrNum, message);
}
...
boolean P_GiveWeapon(player_t *player, weapontype_t weapon)
{
	...
	{ // Cooperative net-game
		...
		player->update |= PSF_OWNED_WEAPONS | PSF_PENDING_WEAPON;
/*		if(player == &players[consoleplayer])
		...
		//S_NetStartPlrSound(player, sfx_wpnup);
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
	...
	/*else
	{
		// Servers are responsible for telling clients to play
		// almost all sounds.
		NetSv_Sound(NULL, sound, player-players);
	}*/
...
void P_KillMobj(mobj_t *source, mobj_t *target)
{
	...
	if(source && source->player)
	{
		...
		{ 
			...
			else
			{
				...
				{
					S_StartSound(sfx_gfrag, NULL);
				}
				else
				{
					NetSv_Sound(NULL, sfx_gfrag, source->player-players);
				}*/
	...
	if(target->player)
	{
		...
		{ // Player flame death
			...
			//S_StartSound(sfx_hedat1, target); // Burn sound
			return;
		}
	}
...
boolean P_AutoUseChaosDevice(player_t *player)
{
	...
	for(i = 0; i < player->inventorySlotNum; i++)
	{
		if(player->inventory[i].type == arti_teleport)
		{
			...
			player->health = player->plr->mo->health = (player->health+1)/2;
			return(true);
		}
	}
	return(false);
