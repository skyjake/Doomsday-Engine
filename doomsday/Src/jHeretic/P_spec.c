...
fixed_t	P_FindNextHighestFloor(sector_t *sec,int currentheight)
{
	...
	for (i=0, h=0; i < sec->linecount; i++)
	{
		...
		if(other->floorheight > currentheight)
		{
			...
			else if(other->floorheight < height)
			{
				// Find the lowest of the surrounding sectors.
				height = other->floorheight;			
			}
		}
	}
...
void P_CrossSpecialLine(int linenum, int side, mobj_t *thing)
{
	...
	switch(line->special)
	{
		...
		//====================================================
		case 2: // Open Door
			EV_DoDoor(line,vd_open,VDOORSPEED);
			line->special = 0;
			break;
		case 3: // Close Door
			EV_DoDoor(line,vd_close,VDOORSPEED);
			line->special = 0;
			break;
		case 4: // Raise Door
			EV_DoDoor(line,vd_normal,VDOORSPEED);
			line->special = 0;
			break;
		case 5: // Raise Floor
	...
	//====================================================
		...
		case 100: // Retrigger_Raise_Door_Turbo
			EV_DoDoor(line, vd_normal, VDOORSPEED*3);
			break;
		case 91:			// Raise Floor
			EV_DoFloor(line,raiseFloor);
			break;
	...
	for(i = 0; i < MAXBUTTONS; i++)
	{
		if(buttonlist[i].btimer)
		{
			...
			if(!buttonlist[i].btimer)
			{
				...
				S_StartSound(sfx_switch, buttonlist[i].soundorg);
