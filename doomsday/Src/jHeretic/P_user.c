...
//
	...
	{
		player->plr->viewz = plrmo->z+player->plr->viewheight-(20*FRACUNIT);
	}
	else
	{
		player->plr->viewz = plrmo->z+player->plr->viewheight;
...
void P_DeathThink(player_t *player)
{
	...
	{
		angle = R_PointToAngle2(plrmo->x, plrmo->y,
...
void P_ClientSideThink()
{
	...
	if(pl->chickenPeck > 0) pl->chickenPeck--;
	...
	if(mo->subsector->sector->special) P_PlayerInWindSector(pl);
