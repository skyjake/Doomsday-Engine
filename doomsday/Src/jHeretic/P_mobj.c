		...
		if(!IS_CLIENT)
		{
			if(mo->momz < 0)
			{
					...
					&& !(mo->flags2&MF2_FLY))       // squat down
				{
					...
					if(/*demorecording || demoplayback || */!cfg.usemlook)
...
// Only needed in co-op games.
void P_DealPlayerStarts()
{
	...
	gi.conprintf( "Player starting spots:\n");
	for(i=0, pl=players; i<MAXPLAYERS; i++, pl++)
	{
		if(!pl->plr->ingame) continue;
		gi.conprintf( "- plr %i, color %i, spot %i\n", i, cfg.PlayerColor[i], 
			pl->startspot);
	}
}*/
...
mobj_t *P_SpawnPlayerMissile(mobj_t *source, mobjtype_t type)
{
	...
	MissileMobj->momx = movfactor * FixedMul(MissileMobj->info->speed,
		finecosine[an>>ANGLETOFINESHIFT]);
	MissileMobj->momy = movfactor * FixedMul(MissileMobj->info->speed,
...
//
	...
	{
		S_StartSound(th->info->seesound, th);
		/*NetSv_Sound(th, th->info->seesound,
			NSSF_NOT | (source->player - players));*/
	}
	th->target = source;
	th->angle = an;
	th->momx = movfactor * FixedMul(th->info->speed, finecosine[an>>ANGLETOFINESHIFT]);
	th->momy = movfactor * FixedMul(th->info->speed, finesine[an>>ANGLETOFINESHIFT]);
