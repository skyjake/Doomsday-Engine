...
void AM_clearFB(int color)
{
	...
	GL_DrawCutRectTiled(0, finit_height + 4, 320, 200 - finit_height - 4, 64,
						64, 160 - 160 * scaler + 1, finit_height,
						320 * scaler - 2, 200 - finit_height);

	GL_SetPatch(lump = W_GetNumForName("bordb"));
	GL_DrawCutRectTiled(0, finit_height, 320, 4, 16, 4, 160 - 160 * scaler + 1,
						finit_height, 320 * scaler - 2, 4);
...
void AM_drawPlayers(void)
{
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		...
		AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0,
							 i ==
							 consoleplayer ? p->plr->clAngle : p->plr->mo->
							 angle, color, p->plr->mo->x, p->plr->mo->y);
	}
	gl.End();
}
