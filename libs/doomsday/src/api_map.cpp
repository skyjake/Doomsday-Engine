...
int DMU_GetType(const void *ptr)
{
    ...
    const world::MapElement *elem = IN_ELEM_CONST(ptr);
...
{
    return world::Map::createDummyElement(type, extraData);
}

dd_bool P_IsDummy(const void *dummy)
{
    return world::Map::dummyElementType(dummy) != DMU_NONE;
}

void P_FreeDummy(void *dummy)
{
    world::Map::destroyDummyElement(dummy);
}

void *P_DummyExtraData(void *dummy)
{
    return world::Map::dummyElementExtraData(dummy);
}
...
int P_ToIndex(const void *ptr)
{
    ...
    const auto *elem = IN_ELEM_CONST(ptr);
    ...
    case DMU_PLANE:
        return elem->as<world::Plane>().indexInSector();
...
void *P_ToPtr(int type, int index)
{
    ...
    {
    case DMU_VERTEX:
        return world::World::get().map().vertexPtr(index);

    case DMU_LINE:
        return world::World::get().map().linePtr(index);

    case DMU_SIDE:
        return world::World::get().map().sidePtr(index);

    case DMU_SECTOR:
        return world::World::get().map().sectorPtr(index);
    ...
    case DMU_SUBSPACE:
        return world::World::get().map().subspacePtr(index);

    case DMU_SKY:
        if(index != 0) return 0; // Only one sky per map, presently.
        return &world::World::get().map().sky();
...
int P_Count(int type)
{
    switch(type)
    {
    case DMU_VERTEX:    return world::World::get().hasMap() ? world::World::get().map().vertexCount()   : 0;
    case DMU_LINE:      return world::World::get().hasMap() ? world::World::get().map().lineCount()     : 0;
    case DMU_SIDE:      return world::World::get().hasMap() ? world::World::get().map().sideCount()     : 0;
    case DMU_SECTOR:    return world::World::get().hasMap() ? world::World::get().map().sectorCount()   : 0;
    case DMU_SUBSPACE:  return world::World::get().hasMap() ? world::World::get().map().subspaceCount() : 0;
...
int P_Iteratep(void *elPtr, uint prop, int (*callback) (void *p, void *ctx), void *context)
{
    ...
    case DMU_SUBSPACE:
        ...
        {
        case DMU_LINE: {
            auto &subspace = elem->as<world::ConvexSubspace>();
            ...
            do
            {
                ...
                {
                    if(int result = callback(&hedge->mapElement().as<world::LineSideSegment>().line(), context))
                        return result;
                }
...
int P_Callback(int type, int index, int (*callback)(void *p, void *ctx), void *context)
{
    ...
    {
    case DMU_VERTEX:
        if (auto *vtx = world::World::get().map().vertexPtr(index))
        {
    ...
    case DMU_LINE:
        if (auto *li = world::World::get().map().linePtr(index))
        {
    ...
    case DMU_SIDE:
        if (auto *si = world::World::get().map().sidePtr(index))
        {
    ...
    case DMU_SUBSPACE:
        if (auto *sub = world::World::get().map().subspacePtr(index))
        {
    ...
    case DMU_SECTOR:
        if (auto *sec = world::World::get().map().sectorPtr(index))
        {
    ...
    case DMU_SKY: {
        ...
        {
            return callback(&world::World::get().map().sky(), context);
        }
        break; }
 ...
 */
static void setProperty(world::MapElement *elem, world::DmuArgs &args)
{
    using world::Sector;
    using world::Line;
    using world::LineSide;
    using world::Plane;
...
static void getProperty(const world::MapElement *elem, world::DmuArgs &args)
{
    using world::Sector;
    using world::Line;
    using world::LineSide;
...
{
    setProperty(IN_ELEM(elPtr), *reinterpret_cast<world::DmuArgs *>(context));
    return false; // Continue iteration.
}

void P_SetBool(int type, int index, uint prop, dd_bool param)
{
    world::DmuArgs args(type, prop);
...
{
    world::DmuArgs args(DMU_GetType(ptr), prop);
    args.valueType = DDVT_BOOL;
    // Make sure invalid values are not allowed.
    param = (param? true : false);
    args.booleanValues = &param;
...
{
    getProperty(IN_ELEM_CONST(elPtr), *reinterpret_cast<world::DmuArgs *>(context));
    return false; // Continue iteration.
}
...
dd_bool P_MapChange(const char *uriCString)
{
    ...
    return world::World::get().changeMap(res::makeUri(uriCString));
}

uint P_CountMapObjs(int entityId)
{
    if(!world::World::get().hasMap()) return 0;
    EntityDatabase &entities = world::World::get().map().entityDatabase();
...
void Mobj_Link(mobj_t *mobj, int flags)
{
    ...
    world::World::get().map().link(*mobj, flags);
}
...
{
    if(!world::World::get().hasMap()) return 0;
    return world::World::get().map().bspLeafAt_FixedPrecision(Vec2d(point)).sectorPtr();
}
    ...
    int (*callback) (mobj_t *, void *), void *context)
{
    ...
    if(world::World::get().hasMap())
    {
        ...
        const int localValidCount = world::World::validCount;
    ...
    int (*callback) (world_Line *, void *), void *context)
{
    ...
    return world::World::get().map().forAllLinesInBox(*box, flags, [&callback, &context] (world::Line &line)
    {
        return LoopResult( callback(&line, context) );
    });
}
    ...
    int (*callback) (struct convexsubspace_s *, void *), void *context)
{
    ...
    return world::World::get().map().subspaceBlockmap()
        .forAllInBox(*box, [&box, &callback, &context, &localValidCount] (void *object)
    {
                         auto &sub = *(world::ConvexSubspace *)object;
    ...
    coord_t topSlope, int flags)
{
    ...
    return world::LineSightTest(Vec3d(from), Vec3d(to), bottomSlope, topSlope, flags)
                .trace(world::World::get().map().bspTree());
}
...
Polyobj *Polyobj_ById(int index)
{
    ...
    return world::World::get().map().polyobjPtr(index);
}
...
Polyobj *Polyobj_ByTag(int tag)
{
    ...
    if(world::World::get().hasMap())
    {
        world::World::get().map().forAllPolyobjs([&tag, &found] (Polyobj &pob)
        {
            if(pob.tag == tag)
            {
                found = &pob;
...
struct mobj_s *Mobj_ById(dint id)
{
    ...
    if (!world::World::get().hasMap()) return nullptr;
    return world::World::get().map().thinkers().mobjById(id);
}
